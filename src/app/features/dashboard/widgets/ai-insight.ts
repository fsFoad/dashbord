...
export class AiInsight {
  ...
  private readonly all = signal<string[]>([]);
  private readonly revealed = signal(0);

  protected readonly visibleInsights = computed(() => this.all().slice(0, this.revealed()));
  ...
  private revealReplay(count: number): void {
    const tick = () => {
      if (this.revealed() >= count) return;
