 ...
 */
let GalleryOverlay = class GalleryOverlay {
    confirmation = inject(ConfirmationService);
    message = inject(MessageService);
    dialog = signal(false);
    drawer = signal(false);
    inplaceText = 'مقدار قابل ویرایش';
    pop = viewChild.required('pop');
    menu = viewChild.required('menu');
    // two-way helpers (PrimeNG dialogs use [(visible)])
    get dialogVisible() { return this.dialog(); }
    set dialogVisible(v) { this.dialog.set(v); }
    get drawerVisible() { return this.drawer(); }
    set drawerVisible(v) { this.drawer.set(v); }
    menuItems = [
