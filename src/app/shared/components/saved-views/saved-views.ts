...
import { SavedView, SavedViewsService } from '../../../core/services/saved-views.service';
...
export class SavedViews<T = unknown> {
  private readonly svc = inject(SavedViewsService);
  private readonly toast = inject(ToastService);

  readonly tableKey = input.required<string>();
  readonly currentState = input.required<T>();

  readonly apply = output<T>();
  ...
  protected readonly views = computed<SavedView<T>[]>(() => {
    this.bump();
    return this.svc.list<T>(this.tableKey());
  });
  ...
  protected applyView(v: SavedView<T>): void {
    this.activeId.set(v.id);
    this.apply.emit(v.state);
  }

  protected del(v: SavedView<T>): void {
