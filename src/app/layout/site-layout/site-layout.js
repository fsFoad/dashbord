 ...
 */
let SiteLayout = class SiteLayout {
    branding = BRANDING;
    settings = inject(SettingsStore);
    language = inject(LanguageService);
    navLinks = resolveSiteMegaMenu();
    mobileOpen = signal(false);
    scrolled = signal(false);
    langLabel = computed(() => this.language.languages.find((l) => l.code === this.settings.language())?.label ?? '');
    currentFlag = computed(() => this.language.languages.find((l) => l.code === this.settings.language())?.flag ?? '🌐');
