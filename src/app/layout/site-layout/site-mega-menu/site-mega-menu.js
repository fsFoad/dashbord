 ...
 */
let SiteMegaMenu = class SiteMegaMenu {
    router = inject(Router);
    host = inject((ElementRef));
    categories = resolveSiteMegaMenu();
    openId = signal(null);
    searchOpen = signal(false);
    query = signal('');
    closeTimer = null;
    activeCategory = computed(() => this.categories.find((c) => c.id === this.openId()) ?? null);
    ...
    results = computed(() => {
        ...
        if (!q)
            return this.allLinks().slice(0, 12);
    ...
    openOnHover(id) {
        this.cancelClose();
        if (this.openId() !== null)
            this.openId.set(id); // only hover-switch if a panel is already open
    }
    scheduleClose() {
