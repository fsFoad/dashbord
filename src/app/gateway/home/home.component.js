...
let HomeComponent = class HomeComponent {
    ...
    hiddenIndexes = [];
    originalData;
    sentry = inject(SentryContextService);
    constructor(route, messagesApiFacadeService, _primengProgressBarService, transloco, apiGatewayService, dialogService, messageService, cdr, ref) {
        /*  this.click$
              .pipe(
    ...
    toggleLabel(index) {
        ...
        dataset.data = dataset.data.map((val, i) => this.hiddenIndexes.includes(i) ? null : val);
        // آپدیت نمودار
   ...
   */
        ...
        this.sentry.setTag('feature', 'home');
    }
    onSelectChange(e) {
    ...
    filterChart() {
        ...
        if (activeIndexes.length === 0) {
            ...
            const chart = this.myChart?.chart;
            if (chart) {
                chart.data.labels = [];
                chart.data.datasets = [];
                chart.update();
        ...
        const filteredLabels = this.originalData.labels.filter((_, i) => activeIndexes.includes(i));
        const filteredDatasets = this.originalData.datasets.map((ds) => ({
            ...ds,
            data: ds.data.filter((_, i) => activeIndexes.includes(i))
        }));
        this.chartDataLineDaily = {
        ...
        if (chart) {
            ...
            chart.data.datasets = filteredDatasets.map((ds) => ({
                ...ds,
                data: [...ds.data]
            }));
            chart.update();
