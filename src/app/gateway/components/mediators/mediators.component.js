...
let MediatorsComponent = class MediatorsComponent {
    route;
    ...
    constructor(route, messagesApiFacadeService, _primengProgressBarService, apiGatewayService, transloco, el, notifierService) {
        this.route = route;
            ...
            if (Array.isArray(parentObj)) {
                parentObj.forEach(item => {
                    if (item && typeof item === 'object')
                        item[keyNode] = valueNode;
                });
            }
            else if (parentObj && typeof parentObj === 'object') {
                parentObj[keyNode] = valueNode;
            }
            else {
    ...
    normalizeTree(nodes, parentPath = '') {
        if (!nodes)
            return;
        nodes.forEach(node => {
            node.dataPath = ((parentPath ? parentPath + '/' : '') + node.key)
                ...
                ?.replace(/\([^)]*\)/g, '');
            if (node.children) {
                this.normalizeTree(node.children, node.dataPath);
            }
        });
    }
    getValueByPath(obj, rawParts) {
    ...
    deleteNode(selectedFileTree2) {
        ...
        try {
            ...
            if (Array.isArray(parentObj)) {
                parentObj.forEach((it) => { if (it && typeof it === 'object')
                    delete it[propKey]; });
            }
            else if (parentObj && typeof parentObj === 'object') {
                delete parentObj[propKey];
            }
            else {
                throw new Error('والد مقصد شیء/آرایه نیست.');
            }
    ...
    showAddValidation() {
        ...
        for (let i = 0; i < this.itemsList.length; i++) {
            if (this.itemsList[i].schemaName == this.tempPath &&
    ...
    uploadFile2(event) {
        ...
        reader.onload = () => {
            try {
                ...
                if (!parsed)
                    throw new Error('xmlToJson هیچ خروجی برنگرداند!');
                this.objs = parsed;
                this.orgObjs = JSON.parse(JSON.stringify(parsed));
                this.files = Object.entries(parsed).map(([rootLabel, rootVal], idx) => this.jsonToTreeNodes(rootVal, rootLabel, '', 0, idx));
    ...
    jsonToTreeNodes(input, label, parentPath, level, order, parent) {
    ...
    setObjSelectedFromCurrentSelection() {
        if (!this.pathSelect) {
            this.objSelected = this.prettyCode(this.objs);
            this.copyObjSelected = this.objSelected;
...
__decorate([
    Input()
], MediatorsComponent.prototype, "inputListMedia", void 0);
__decorate([
    Output()
], MediatorsComponent.prototype, "close", void 0);
__decorate([
    ViewChild('myTree')
], MediatorsComponent.prototype, "myTree", void 0);
__decorate([
    ViewChild('cm')
], MediatorsComponent.prototype, "cm", void 0);
MediatorsComponent = __decorate([
    Component({
