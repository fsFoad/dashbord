...
let InputMediatorComponent = class InputMediatorComponent {
    ...
    clientListOptions = [];
    selectedClientIds = []; // یا string[] بسته به نوع clientId
    ...
    jsonPlaceholder = `
{
  ...
  "token": "secret123"
    ...
    saveReportActionLines = [];
    saveReportActionItems = [];
    rules = [];
    ...
    isFirstPreviewAfterLoad = true;
    originalJsonOrder = null;
    operatorToNumberMap = {
        '1': 1,
        '2': 2,
        '3': 3,
    ...
    buildValidatorExpression(rule) {
        const node = (rule.path || '').trim() || 'node';
        const getLabel = (code) => this.getConditionLabel(rule, code);
        // =========================
        // 🔥 STRING
        // =========================
        ...
        if (mode === 'single') {
            ...
            if (!label || !rule.compareTargetValue)
                return null;
    ...
    getUniqueNewNodeName(base = 'newNode') {
        ...
        return {
            ...
            inputMediatorElementDtoList: this.rules
                ...
                const dto = {
                    ...
                    outputJsonTest: this.applyDeleteOnOutputJson(this.jsonInput, this.rules),
                    // ========= SOURCE =========
                    sourceNodeName: rule.kind === 'new'
                    ...
                    detailOperationFlag: rule.deleted === true
                        ? 'D'
                        : (rule.mediatorId ? 'U' : 'I'),
                    ...
                    targetNodeMappingValue: rule.action == 1
                        ? (rule.mapTo ?? null)
                        : null,
                    targetNodeCalculationScript: rule.action == 2
                        ? this.prepareJsForBackend(rule.jsCode)
                        : null,
                    targetNodeEncode: rule.finalEncode ? 1 : 0,
                    targetNodeExtraValueNodePath: rule.serviceResponsePath?.trim()
                        ? rule.serviceResponsePath
                        : null,
                    targetNodeCalledApiId: rule.action == 3
                        ? (rule.apiId ?? null)
                        : null,
                    targetNodeCalledPartyId: rule.action == 3
                        ? (rule.partyId ?? null)
                        : null,
                    targetNodeCalledModuleId: rule.action == 3
                        ? (rule.moduleId ?? null)
                        : null,
                    participateTargetNodeAsForwardEndpointDetail: rule.sendNodeAsQueryParam === 1 ? 1 : 0,
    ...
    rebuildWithOrder(order, source) {
        if (!order)
            return source;
        // array
        if (Array.isArray(order) && Array.isArray(source)) {
            return source.map((v, i) => this.rebuildWithOrder(order[i], v));
        }
        // object
        if (Array.isArray(order) && typeof source === 'object') {
            ...
            for (const item of order) {
                if (source.hasOwnProperty(item.key)) {
                    result[item.key] = this.rebuildWithOrder(item.children, source[item.key]);
                }
            }
    ...
    mapBackendToRules(list) {
        ...
        return (list || []).map(item => {
            ...
            // 🔥 تشخیص new / existing
            // =========================
            const existsInInput = this.lastParsedInput &&
            ...
            if (item.targetNodeActionType === 2 && item.targetNodeCalculationScript) {
                rule.action = 2;
                rule.jsCode = this.sanitizeJsCode(item.targetNodeCalculationScript);
            }
            return rule;
        });
    }
    ngOnInit() {
        ...
        forkJoin({
            ...
            return this.messagesApiFacadeService.getInputMediatorElementDetail(this.inputMediatorApi.apiId, groupId);
        }), tap(res => {
            const body = res?.body;
            if (!body)
                return;
                            ...
                            this.messagesApiFacadeService
                                ...
                                if (rule.moduleId) {
                                    this.messagesApiFacadeService
                                        ...
                                        const selectedApi = apis.find(x => x.apiId === rule.apiId);
                                        rule.apiTitle =
    ...
    buildAvailablePaths() {
        ...
        if (!data)
            return [];
        const paths = [];
        const isObjectOrArray = (v) => v !== null && (Array.isArray(v) || typeof v === 'object');
        const walk = (val, path) => {
...
return result;`,
    ...
    getJsonRawValue(rule) {
        if (!this.lastParsedInput)
            return '';
    ...
    onChangeParty(rule, event) {
        ...
        if (selectedPartyId != null) {
            ...
            this.messagesApiFacadeService.moduleSearchByPartyId(selectedPartyId).subscribe((m) => {
                this._primengProgressBarService.hide();
                rule.moduleId = null;
    ...
    runJsTransform(rule, value, context) {
        const code = (rule.jsCode ?? '').trim();
        if (!code)
            return value;
        const fn = new Function('value', 'rule', 'ctx', `
    "use strict";
    ${code}
    `);
        return fn(value, rule, context);
    }
    applyJsRule(rule, baseValue) {
        try {
            const ctx = {
                json: this.lastParsedInput, // json ورودی
                path: (rule.path || '').trim(), // مسیر نود
                targetPath: (rule.targetPath || '').trim(),
                getByPath: (p) => this.getByPath(this.lastParsedInput, p),
                // اگر خواستی encode/decode هم بده
                b64Encode: (s) => this.b64Encode(s),
                b64Decode: (s) => this.b64Decode(s),
            };
    ...
    flattenPrimitiveValues(val, out = []) {
        if (val === null || val === undefined)
            return out;
        if (Array.isArray(val)) {
            for (const item of val)
                this.flattenPrimitiveValues(item, out);
            return out;
        }
        if (typeof val === 'object') {
            for (const k of Object.keys(val))
                this.flattenPrimitiveValues(val[k], out);
            return out;
        }
        // primitive
        out.push(String(val));
    ...
    onChangeModule(rule, event) {
        ...
        if (selectedModuleId != null) {
            ...
            this._primengProgressBarService.show();
            this.messagesApiFacadeService.apiNochart(0, 10000, selectedModuleId).subscribe((apis) => {
                this._primengProgressBarService.hide();
    ...
    sortDeepKeysKeepArrays(x, isRoot = false) {
        if (Array.isArray(x)) {
    ...
    getDisplayedOriginalValue(rule) {
    ...
    onAutoDecodeToMapFromToggle(rule, ev) {
        rule.autoDecodeToMapFrom = ev?.checked === true;
        this.updateOutputPreview();
    }
    getRawOriginalValue(rule) {
        if (!this.lastParsedInput)
            return '';
        const basePath = (rule.path || '').trim();
        if (!basePath)
    ...
    getServiceTitle(rule) {
        return rule?.apiTitle?.trim() ? rule.apiTitle : 'node';
    }
    getFinalPreviewValue(rule, includeAffixes = true) {
    ...
    applyPathTransform(obj, fromPath, toPath, value) {
    ...
    applyDeleteOnOutputJson(rawJson, rules) {
        try {
    ...
    updateOutputPreview() {
        ...
        try {
            ...
            if (this.autoGenerateRules && fp !== this.lastJsonFingerprint) {
                ...
                const oldMap = new Map(existingRules.map(r => [this.normalizeName(r.path), r]));
            ...
            // NO CHANGE
            // =========================
            const hasAnyChange = this.rules.some(r => r.kind === 'new' || this.detectOpsForRule(r).size > 0);
            ...
            for (const rule of this.rules) {
                ...
                if (ops.size === 0 && rule.deleted !== true)
                    continue;
                const fromPath = (rule.path || '').trim();
                const toPath = (rule.targetPath || '').trim() ||
                    (rule.newPath || '').trim() ||
                ...
                // =========================
                if (rule.action === 3) {
                    const serviceName = (rule.apiTitle || '').trim() ||
    ...
    getByPath(obj, path) {
        ...
        for (const p of parts) {
            ...
            if (typeof cur === 'object' && !Array.isArray(cur)) {
                const key = Object.keys(cur).find(k => this.normalizeName(k) === this.normalizeName(String(p)));
    ...
    setByPath(obj, path, value) {
        ...
        for (let i = 0; i < parts.length - 1; i++) {
            const p = parts[i];
            let key = Object.keys(cur).find(k => this.normalizeName(k) === this.normalizeName(String(p)));
            if (!key) {
                key = p;
                cur[key] = {};
            }
    ...
    detectOpsForRule(rule) {
        ...
        // 4) تغییر تایپ
        if (rule.originalType &&
            rule.newType &&
            rule.newType !== rule.originalType) {
            ops.add('typeChanged');
        }
        // 5) شرط و محاسبات
        const numberCondActive = rule.newType === 'Number' &&
            rule.numberConditionMode &&
            rule.numberConditionMode !== 'none';
        const nonNumberCondActive = rule.newType !== 'Number' &&
            !!(rule.ruleCondition &&
                String(rule.compareTargetValue ?? '').trim());
        if ((numberCondActive || nonNumberCondActive) &&
            rule.action !== 0) {
            ops.add('conditionCalc');
        }
        // 6) Decode
    ...
    ApiGatewayConstants = ApiGatewayConstants;
};
__decorate([
    Input()
], InputMediatorComponent.prototype, "inputMediatorApi", void 0);
__decorate([
    Output()
], InputMediatorComponent.prototype, "close", void 0);
InputMediatorComponent = __decorate([
    Component({
