...
import { ChangeDetectionStrategy, ChangeDetectorRef, Component, computed, EventEmitter, inject, Input, Output, signal, } from '@angular/core';
...
let AggregatorConfigComponent = class AggregatorConfigComponent {
    ...
    form = this.fb.group({
        aggregatorTitle: this.fb.control(''),
        responseMode: this.fb.control('all'),
        clientIds: this.fb.control([]),
        statusCode: this.fb.control(200),
        genElement: this.fb.control(null),
        reqElement: this.fb.control(null),
        services: this.fb.array([]),
        outputFields: this.fb.array([]),
    });
    apiTitle;
    curlInput = signal('');
    curlError = signal(null);
    curlValid = signal(false);
    parsedCurl = signal(null);
    curlDialogVisible = signal(false);
    selectedCurlService = signal(null);
    services = this.form.controls
        .services;
    outputFields = this.form.controls
        .outputFields;
    mappingDrafts = {};
    sourceServices = computed(() => {
        ...
        return [
            ...
            ...this.services.controls.map((service, index) => ({
                label: this.getSelectedServiceTitle(service),
                value: service.controls.id.value,
            })),
        ];
    });
    targetServices = computed(() => {
        return this.services.controls.map((service) => {
            const selectedApiId = service.controls.service.value;
            const apiOptions = service.controls.apiOptions.value || [];
            const selectedApi = apiOptions.find(x => x.apiId === selectedApiId);
            return {
                label: selectedApi?.title || '-',
                value: selectedApi?.apiId || null,
            };
        });
    });
    statusCodeSignal = signal(200);
    previewJson = computed(() => {
        // refresh triggers
        this.outputRefreshSignal();
        this.servicesRefreshSignal();
        this.services.length;
        const statusCode = this.statusCodeSignal();
        // =========================
        // SUCCESS MODE (200)
        // =========================
        if (statusCode === 200) {
            const outputValues = this.outputFields.getRawValue();
            const groupedOutputs = {};
            outputValues.forEach((value) => {
                const selectedService = this.sourceServices()
                    .find(x => x.value === value.sourceId);
                const serviceTitle = selectedService?.label || '-';
                // create service group
                if (!groupedOutputs[serviceTitle]) {
                    groupedOutputs[serviceTitle] = {};
                }
                // append field
                groupedOutputs[serviceTitle][value.key || 'field'] =
                    value.sourcePath
            ...
            return {
                code: statusCode,
                output: Object.entries(groupedOutputs)
                    .map(([key, value]) => ({
                    [key]: value,
                })),
            };
        ...
        this.services.controls.forEach(service => {
            const serviceTitle = this.getSelectedServiceTitle(service);
            const finalTitle = serviceTitle &&
                serviceTitle !== 'سرویس'
                ? serviceTitle
                : '-';
            groupedErrors[`سرویس (${finalTitle})`] = {
                code: statusCode,
                output: {
                    response: `ریسپانس سرویس (${finalTitle})`,
                },
            };
        });
        return groupedErrors;
    });
    servicesRefreshSignal = signal(0);
    outputRefreshSignal = signal(0);
    isSuccessResponse = computed(() => this.statusCodeSignal() === 200);
    isCreateMode = false;
    ...
    modeFlag;
    detailsBreadObject = [];
    sourceElementOptions = [];
    cdr = inject(ChangeDetectorRef);
    partyListOptions = [{ title: '-', partyId: null }];
    curlBodyPreview = signal({});
    constructor(_primengProgressBarService, messagesApiFacadeService, notifierService, apiGatewayService, transloco, confirmationService) {
    ...
    getLineAndColumn(text, position) {
        const lines = text.substring(0, position)
            .split('\n');
        return {
            line: lines.length,
    ...
    buildServicesFromCurl(curl) {
        ...
        const normalized = decoded
            ...
            .replace(/\s+/g, ' ')
            .trim();
        const bodyMatch = decoded.match(/(?:^|\s)(?:-d|--data(?:-raw|-binary|-urlencode)?)\s+(["'])([\s\S]*?)\1/);
    ...
    populateAggregator(dto) {
        console.log('DTO', dto);
        console.log('INPUT CURL', dto?.inputCurl);
        console.log('FULL RESPONSE DTO', JSON.stringify(dto, null, 2));
        const list = (dto?.aggregateServiceInfoDtoList ?? [])
            .filter((x) => x.status === 1);
        if (!list.length) {
            return;
        }
        ...
        // --- inputCurl ---
        if (dto.inputCurl) {
            let decodedCurl = this.fromBase64Unicode(dto.inputCurl);
            // اگر رشته stringify شده ذخیره شده بود
            try {
                if (decodedCurl.startsWith('"') &&
            ...
            this.curlInput.set(decodedCurl);
            try {
                const parsed = this.buildServicesFromCurl(decodedCurl);
                console.log('DECODED CURL', decodedCurl);
                console.log('PARSED BODY', parsed.body);
                this.curlRequestBody.set(parsed?.body ?? null);
            }
            catch {
        ...
        const firstClients = list[0]?.clientInfoList ?? [];
        const checkedClientIds = firstClients
            .filter((c) => c.status === 1)
            .map((c) => c.clientId);
        this.clientConfigs =
            firstClients.map(c => ({
        ...
        this.form.controls.clientIds.setValue(firstClients
            .filter(c => c.status === 1)
            .map(c => c.clientId), { emitEvent: false });
        this.form.controls.clientIds.setValue(checkedClientIds, {
            emitEvent: false,
        });
        // --- بازسازی سرویس‌ها ---
        this.services.clear();
        Object.keys(this.mappingDrafts).forEach(k => delete this.mappingDrafts[+k]);
        // به ازای هر آیتم یک کادر سرویس
        list.forEach((item) => {
            this.addService();
            const index = this.services.length - 1;
            const serviceForm = this.services.at(index);
            this.buildServiceFromDto(serviceForm, item, index);
        });
    ...
    buildServiceFromDto(serviceForm, item, index) {
        console.log('BUILD SERVICE DTO =>', item.id, item.aggregateServiceResponseDtoList);
        ...
        this.messagesApiFacadeService
            ...
            this.messagesApiFacadeService
                ...
                this.searchNotIsSystemByApiId(apiId).subscribe(res => {
                    ...
                    this.buildResponsesFromDto(serviceForm, item);
                    serviceForm.patchValue({
                        outputJsonRootNodeName: item.outputJsonRootNodeName ?? '',
                    }, {
                        emitEvent: false,
                    });
                    this.servicesRefreshSignal.update(x => x + 1);
    ...
    buildConnectionsFromDto(serviceForm, item, destApiId) {
        const connections = this.getConnections(serviceForm);
        connections.clear();
        (item.aggregateServiceMapDtoList ?? []).forEach((map) => {
            connections.push(this.createConnectionForm({
                ...
                genElementInputName: map.srcInputEndpointDetailInputName,
                reqServiceId: destApiId,
                reqElement: map.destInputEndpointDetailId,
                reqElementInputName: map.destInputEndpointDetailInputName,
            }));
        });
    }
    ...
    importCurl() {
        ...
        const parsed = this.buildServicesFromCurl(this.curlInput());
        this.curlRequestBody.set(parsed?.body ?? null);
        this.closeCurlDialog();
        const result = this.parseCurl(this.curlInput());
        console.log('RESULT', result);
        console.log('BODY', result.body);
        this.curlRequestBody.set(result.body);
        console.log('SIGNAL', this.curlRequestBody());
    }
    openSourceElementDialog() {
    ...
    openDestinationElementDialog(serviceIndex) {
        const selectedApiId = this.services
            .at(serviceIndex)
            .controls
            .service
            .value;
        if (!selectedApiId) {
            this.notifierService.showWarning({
                detail: 'لطفا سرویس مقصد را انتخاب کرده و مجدد افزودن المان را کلیک کنید!',
                life: 3000,
            });
            return;
        }
    ...
    onSourceElementDialogClose(event) {
    ...
    onDestinationElementDialogClose(event) {
        this.destinationElementDialogVisible = false;
        if (event === 'closeAndCreate') {
            this.services.controls.forEach(service => {
                const targetApiId = service.controls.service.value;
                if (!targetApiId) {
                    return;
                }
 ...
 `;
    ...
    validateCurlLive(value) {
        ...
        try {
            ...
            this.curlValid.set(false);
            this.curlError.set(error?.message || 'فرمت cURL نامعتبر است');
        }
    }
    BeforeButton() {
        this.close.emit('close');
    }
    searchNotIsSystemByApiId(apiId) {
        return this.messagesApiFacadeService
            .endpointdetailByApi(apiId, 0, 110000, 0, {
            skipEmptyCheck: true,
        })
            .pipe(map((response) => {
            let data = [];
    ...
    ngOnInit() {
        ...
        this.outputFields.valueChanges
            .subscribe(() => {
            this.outputRefreshSignal.update(x => x + 1);
        });
        if (this.inputAggregatorConfig != undefined) {
            ...
            if (this.inputAggregatorConfig.operationFlag === 'U') {
                ...
                this.messagesApiFacadeService.getAggregate(this.inputAggregatorConfig.aggregateId).subscribe(res => {
                    ...
                    debugger;
                    this.loadAggregateForEdit(this.aggregateDto);
                });
            }
        }
        this.searchNotIsSystemByApiId(this.apiId)
    ...
    storePreviousValue(serviceForm, controlName) {
        const formId = serviceForm.controls.id.value;
        const key = `${formId}_${controlName}`;
        this.previousSelections.set(key, serviceForm.controls[controlName].value);
    }
    addOutput200(serviceIndex) {
        const service = this.services.at(serviceIndex);
        const output = this.createOutputField();
        output.patchValue({
    ...
    removeOutput200(serviceIndex, outputIndex) {
        const service = this.services.at(serviceIndex);
        if (!service) {
            return;
        }
        const outputs = this.getOutputs200(service);
        if (!outputs) {
            return;
        }
        const output = outputs.at(outputIndex);
        if (!output) {
            console.warn('Output not found', serviceIndex, outputIndex);
            return;
        }
        if (this.modeFlag === 'U' &&
            output.get('responseConfigId')?.value) {
    ...
    removeOutput400(serviceIndex, outputIndex) {
        const service = this.services.at(serviceIndex);
        if (!service) {
            return;
        }
        const outputs = this.getOutputs400(service);
        if (!outputs) {
            return;
        }
        const output = outputs.at(outputIndex);
    ...
    getOutputs200(service) {
        return service.controls.outputs200;
    }
    getOutputs400(service) {
        return service.controls.outputs400;
    }
    getVisibleOutputs400(service) {
        return this.getOutputs400(service)
            .controls
            .filter(x => x.get('status')?.value !== 0);
    }
    getOutputs401(service) {
        return service.controls.outputs401;
    }
    getOutputs500(service) {
        return service.controls.outputs500;
    }
    getTargetElementOptions(apiId) {
        if (!apiId) {
            return [];
        }
        return this.targetElementOptions[apiId] ?? [];
    }
    getElementTitleById(elementId, apiId) {
        if (!elementId) {
            return '-';
        }
        // مبدا
        const sourceElement = this.sourceElementOptions.find(x => x.value === elementId);
        if (sourceElement) {
            return sourceElement.label;
        }
        // مقصد
        if (apiId) {
            const targetElement = (this.targetElementOptions[apiId] || [])
                .find(x => x.value === elementId);
            if (targetElement) {
    ...
    onChangeModule(serviceForm, event) {
        const selectedModuleId = event.value ?? event;
        const formId = serviceForm.controls.id.value;
        const previousModuleId = this.previousSelections.get(`${formId}_module`);
        const previousServiceId = serviceForm.controls.service.value;
        const previousApiOptions = serviceForm.controls.apiOptions.value;
        const serviceIndex = this.services.controls.findIndex(x => x === serviceForm);
        const selectedModule = serviceForm.controls.moduleOptions.value.find(x => x.moduleId === selectedModuleId);
        const applyChange = () => {
            ...
            this.clearMappings(serviceForm, serviceIndex);
            ...
            this._primengProgressBarService.show();
            this.messagesApiFacadeService
                .apiNochart(0, 10000, selectedModuleId)
                .subscribe({
                next: apis => {
                    serviceForm.patchValue({
                        apiOptions: [
        ...
        if (this.hasMappings(serviceForm)) {
            this.confirmationService.confirm({
                ...
                header: 'تغییر ماژول',
                message: `تغییر ماژول موجب از دست رفتن اطلاعات Mapping Engine می‌شود.`,
                accept: () => {
                    applyChange();
                },
                reject: () => {
                    serviceForm
                        .get('module')
                        ?.setValue(previousModuleId, {
                        emitEvent: false,
                    });
                    serviceForm
                        .get('service')
                        ?.setValue(previousServiceId, {
                        emitEvent: false,
                    });
                    serviceForm.patchValue({
    ...
    getSelectedServiceTitle(serviceForm) {
        const selectedApiId = serviceForm.controls.service.value;
        const apiOptions = serviceForm.controls.apiOptions.value || [];
        const selectedApi = apiOptions.find(x => x.apiId === selectedApiId);
        return ((!selectedApi?.title || selectedApi?.title == '-') ? 'سرویس' : selectedApi?.title);
    }
    onChangeParty(serviceForm, event) {
        const selectedPartyId = event.value ?? event;
        const formId = serviceForm.controls.id.value;
        const previousPartyId = this.previousSelections.get(`${formId}_org`);
        const previousModuleId = serviceForm.controls.module.value;
        const previousServiceId = serviceForm.controls.service.value;
        const previousModuleOptions = serviceForm.controls.moduleOptions.value;
        const previousApiOptions = serviceForm.controls.apiOptions.value;
        const serviceIndex = this.services.controls.findIndex(x => x === serviceForm);
        const selectedParty = this.partyListOptions.find(x => x.partyId === selectedPartyId);
        const applyChange = () => {
            const draft = this.mappingDrafts[serviceIndex];
            const sourceValues = {
                genServiceId: draft?.get('genServiceId')?.value,
        ...
        if (this.hasMappings(serviceForm)) {
            this.confirmationService.confirm({
                key: 'mappingResetDialog',
                header: 'تغییر سازمان',
                message: `تغییر سازمان موجب از دست رفتن اطلاعات Mapping Engine می‌شود.`,
                accept: () => {
                    applyChange();
                },
                reject: () => {
    ...
    onChangeApi(serviceForm, event) {
        const formId = serviceForm.controls.id.value;
        const previousApiId = this.previousSelections.get(`${formId}_service`);
        const selectedApiId = event.value ?? event;
        // جلوگیری از انتخاب سرویس تکراری
        const duplicated = selectedApiId &&
            this.services.controls.some(service => service !== serviceForm &&
                service.controls.service.value === selectedApiId);
        if (duplicated) {
            this.notifierService.showError({
                detail: 'امکان انتخاب این سرویس نیست، این سرویس قبلا انتخاب شده است!',
                life: 3000,
            });
            serviceForm
                .get('service')
                ?.setValue(previousApiId, {
                emitEvent: false,
            });
            return;
        }
        ...
        const applyChange = () => {
            const draft = this.mappingDrafts[serviceIndex];
            const genElement = draft?.get('genElement')?.value;
            const genPath = draft?.get('genPath')?.value;
            this.clearMappings(serviceForm, serviceIndex);
            draft?.patchValue({
                genElement,
                genPath,
        ...
        if (this.hasMappings(serviceForm)) {
            this.confirmationService.confirm({
                key: 'mappingResetDialog',
                header: 'تغییر سرویس',
                message: 'تغییر سرویس موجب از دست رفتن اطلاعات Mapping Engine می‌شود.',
                accept: () => {
                    applyChange();
                },
                reject: () => {
    ...
    createServiceForm() {
        return this.fb.group({
            ...
            moduleOptions: this.fb.control([
                {
                    moduleTitle: '-',
                    moduleId: null,
                },
            ]),
            apiOptions: this.fb.control([
                {
                    title: '-',
                    apiId: null,
                },
    ...
    clientListOptions = [];
    createConnectionForm(data) {
        return this.fb.group({
            // ===== Edit Fields =====
            inputConfigId: this.fb.control(data?.inputConfigId ?? null),
            aggregateServiceId: this.fb.control(data?.aggregateServiceId ?? null),
            status: this.fb.control(data?.status ?? 1),
            // ===== Mapping Fields =====
            genType: this.fb.control({
                value: 'body',
                disabled: true,
            }),
            genServiceId: this.fb.control(data?.genServiceId || 'global'),
            // Id مبدا
            genElement: this.fb.control(data?.genElement || ''),
            // نام المان مبدا
            genElementInputName: this.fb.control(data?.genElementInputName ?? null),
            genPath: this.fb.control(data?.genPath || ''),
            reqType: this.fb.control({
                value: 'body',
                disabled: true,
            }),
            reqServiceId: this.fb.control(data?.reqServiceId || ''),
            // Id مقصد
            reqElement: this.fb.control(data?.reqElement || ''),
            // نام المان مقصد
            reqElementInputName: this.fb.control(data?.reqElementInputName ?? null),
            reqPath: this.fb.control(data?.reqPath || ''),
        });
    }
    clearServices() {
        this.services.clear();
    ...
    hasMappings(serviceForm) {
        return this.getConnections(serviceForm)
            .controls.length > 0;
    }
    clearMappings(serviceForm, serviceIndex) {
    ...
    validateBeforeSave() {
        ...
        // Client
        // =========================
        const clientIds = this.form.controls.clientIds.value;
        if (!clientIds?.length) {
            this.notifierService.showError({
        ...
        for (let index = 0; index < this.services.length; index++) {
            ...
            const serviceNo = index + 1;
            // سازمان مقصد
            if (!service.controls.org.value) {
                this.notifierService.showError({
                    detail: `لطفا سازمان مقصد را برای سرویس شماره ${serviceNo} وارد نمایید!`,
                    life: 4000,
                });
                return false;
            }
            ...
            if (!service.controls.module.value) {
                this.notifierService.showError({
                    detail: `لطفا ماژول مقصد را برای سرویس شماره ${serviceNo} را وارد نمایید!`,
                    life: 4000,
                });
                return false;
            }
            ...
            if (!service.controls.service.value) {
                this.notifierService.showError({
                    detail: `لطفا سرویس مقصد را برای سرویس شماره ${serviceNo} را وارد نمایید!`,
                    life: 4000,
                });
                return false;
            }
            ...
            // =========================
            const targetApiId = service.controls.service.value;
            const allElements = this.targetElementOptions[targetApiId] ?? [];
            const remainingElements = this.getAvailableTargetElements(service, targetApiId);
            if (allElements.length > 0 &&
                remainingElements.length > 0) {
                this.notifierService.showError({
                    detail: `سرویس شماره ${serviceNo} المان مپ نشده دارد، لطفا المان را به لیست مپینگ اضافه کنید!`,
                    life: 4000,
                });
                return false;
            }
            ...
            // Mapping Validation
            // =========================
            const mappings = this.getConnections(service)
                .controls
                .filter(x => x.get('status')?.value !== 0);
            if (!mappings.length) {
                this.notifierService.showError({
                    detail: `هیچ مشخصاتی برای مپ‌کردن المان‌ها برای سرویس شماره ${serviceNo} وارد نشده است.`,
                    life: 4000,
                });
                return false;
            }
            ...
            if (!service.controls
                ...
                .value?.trim()) {
                this.notifierService.showError({
                    detail: `لطفا نام روت خروجی سرویس شماره ${serviceNo} را وارد کنید!`,
                    life: 4000,
                });
                return false;
            }
            ...
            if (!service.controls
                ...
                .value?.trim()) {
                this.notifierService.showError({
                    detail: `لطفا برای خروجی وضعیت 401 سرویس شماره ${serviceNo} خروجی جیسون استاندارد وارد کنید!`,
                    life: 4000,
                });
                return false;
            }
            ...
            if (!service.controls
                ...
                .value?.trim()) {
                this.notifierService.showError({
                    detail: `لطفا برای خروجی وضعیت 500 سرویس شماره ${serviceNo} خروجی جیسون استاندارد وارد کنید!`,
                    life: 4000,
                });
                return false;
            }
            ...
            // 200 Selected
            // =========================
            if (service.controls.responseMode200.value
                === 'selected') {
                const outputs200 = this.getOutputs200(service);
                if (!outputs200.length) {
                    this.notifierService.showError({
                        detail: `لطفا یک نود منتخب وضعیت 200 برای سرویس شماره ${serviceNo} وارد کنید!`,
                        life: 4000,
                    });
                    return false;
                }
                ...
                const invalidIndex = outputs200.controls.findIndex((x) => !x.controls.sourcePath.value?.trim());
                if (invalidIndex !== -1) {
                    this.notifierService.showError({
                        detail: `لطفا مسیر نود منتخب شماره ${invalidIndex + 1} وضعیت 200 برای سرویس شماره ${serviceNo} وارد کنید!`,
                        life: 4000,
                    });
                    return false;
                }
            ...
            // 400 Selected
            // =========================
            if (service.controls.responseMode400.value
                === 'selected') {
                const outputs400 = this.getOutputs400(service);
                if (!outputs400.length) {
                    this.notifierService.showError({
                        detail: `لطفا یک نود منتخب وضعیت 400 برای سرویس شماره ${serviceNo} وارد کنید!`,
                        life: 4000,
                    });
                    return false;
                }
                ...
                const invalidIndex = outputs400.controls.findIndex((x) => !x.controls.sourcePath.value?.trim());
                if (invalidIndex !== -1) {
                    this.notifierService.showError({
                        detail: `لطفا مسیر نود منتخب شماره ${invalidIndex + 1} وضعیت 400 برای سرویس شماره ${serviceNo} وارد کنید!`,
                        life: 4000,
                    });
                    return false;
                }
    ...
    addService() {
        const serviceForm = this.createServiceForm();
        ...
        this.mappingDrafts[index]
            .get('reqType')
            ?.disable();
        const control = this.mappingDrafts[index]
            .get('genServiceId');
    ...
    addConnection(serviceIndex) {
        ...
        const value = draft.getRawValue();
        // ==========================
        // Source Element Name
        // ==========================
        const sourceElement = this.sourceElementOptions.find(x => x.value === value.genElement);
        // ==========================
        // Target Element Name
        // ==========================
        const targetElement = (this.targetElementOptions[value.reqServiceId] || []).find(x => x.value === value.reqElement);
        const connectionValue = {
            ...value,
            genElementInputName: sourceElement?.label ?? null,
            reqElementInputName: targetElement?.label ?? null,
        };
        this.getConnections(this.services.at(serviceIndex)).push(this.createConnectionForm(connectionValue));
        const currentServiceId = this.services
            .at(serviceIndex)
            .controls
            .service
            .value;
    ...
    getServiceTitleById(apiId) {
        ...
        for (const service of this.services.controls) {
            const apiOptions = service.controls.apiOptions.value || [];
            const found = apiOptions.find(x => x.apiId === apiId);
            if (found) {
                return found.title;
            }
        }
    ...
    buildResponsesFromDto(serviceForm, item) {
        console.log('CALLING buildResponsesFromDto');
        console.log('ENTER buildResponsesFromDto', item.aggregateServiceResponseDtoList);
        const responses = (item.aggregateServiceResponseDtoList ?? [])
            .filter((x) => x.status === 1);
        // =========================
        // 200
        // =========================
        const outputs200 = serviceForm.controls.outputs200;
        outputs200.clear();
        const rows200 = responses.filter((x) => x.responseStatusCode === 200);
        const selected200 = rows200.some((x) => x.responsePartType === 1);
        const row200ConfigId = rows200.find((x) => x.responseConfigId != null)
        ...
        serviceForm.controls.responseMode200
            .setValue(selected200
            ? 'selected'
            : 'all', {
            emitEvent: false,
        });
        serviceForm.controls.isArray200
            .setValue(rows200.some((x) => x.isArray === 1), {
            emitEvent: false,
        });
        if (selected200) {
            rows200.forEach((row) => {
                const field = this.createOutputField();
                console.log('ROW 200 =>', row);
                field.patchValue({
                    sourceId: serviceForm.controls.id.value,
                    responseConfigId: row.responseConfigId,
                    aggregateServiceId: row.aggregateServiceId,
                    status: row.status,
                    sourcePath: row.responseNodePath ?? '',
                    key: row.responseNewNodeName ?? '',
                }, {
                    emitEvent: false,
                });
                console.log('200FIELD AFTER PATCH =>', field.getRawValue());
                outputs200.push(field);
            });
        }
        // =========================
        // 400
        // =========================
        const outputs400 = serviceForm.controls.outputs400;
        outputs400.clear();
        const rows400 = responses.filter((x) => x.responseStatusCode === 400);
        const selected400 = rows400.some((x) => x.responsePartType === 1);
        const row400ConfigId = rows400.find((x) => x.responseConfigId != null)
            ?.responseConfigId ?? null;
        serviceForm.controls.responseConfigId400
            .setValue(row400ConfigId, { emitEvent: false });
        serviceForm.controls.responseMode400
            .setValue(selected400
            ? 'selected'
            : 'all', {
            emitEvent: false,
        });
        serviceForm.controls.isArray400
            .setValue(rows400.some((x) => x.isArray === 1), {
            emitEvent: false,
        });
        if (selected400) {
            rows400.forEach((row) => {
                const field = this.createOutputField();
                console.log('ROW 400 =>', row);
                field.patchValue({
                    sourceId: serviceForm.controls.id.value,
                    responseConfigId: row.responseConfigId,
                    aggregateServiceId: row.aggregateServiceId,
                ...
                console.log('400FIELD AFTER PATCH =>', field.getRawValue());
                outputs400.push(field);
                console.log('RESPONSES =>', item.aggregateServiceResponseDtoList);
            });
        }
        // =========================
        // 401
        // =========================
        const response401 = responses.find(x => x.responseStatusCode === 401);
        serviceForm.patchValue({
        ...
        if (response401?.outputStaticJson) {
            serviceForm.patchValue({
                outputStaticJson401: this.fromBase64Unicode(response401.outputStaticJson),
            }, {
                emitEvent: false,
            });
        }
        ...
        // 500
        // =========================
        const response500 = responses.find(x => x.responseStatusCode === 500);
        serviceForm.patchValue({
        ...
        if (response500?.outputStaticJson) {
            serviceForm.patchValue({
                outputStaticJson500: this.fromBase64Unicode(response500.outputStaticJson),
            }, {
                emitEvent: false,
            });
        }
        ...
        if (serviceIndex !== -1) {
            this.jsonValidation401[serviceIndex] =
                this.validateJsonInternal(serviceForm.controls
                    .outputStaticJson401
                    .value);
            this.jsonValidation500[serviceIndex] =
                this.validateJsonInternal(serviceForm.controls
        ...
        console.log('ALL RESPONSES =>', item.aggregateServiceResponseDtoList);
        console.log('ROWS200 =>', rows200);
        console.log('ROWS400 =>', rows400);
        console.log('SELECTED200 =>', selected200);
        console.log('SELECTED400 =>', selected400);
        this.cdr.markForCheck();
    }
    validateJson401(value, index) {
        this.jsonValidation401[index] =
            this.validateJsonInternal(value);
        this.cdr.markForCheck();
    }
    validateJson500(value, index) {
    ...
    removeConnection(serviceIndex, connection) {
        if (!connection) {
            return;
        }
        if (this.modeFlag === 'U' &&
        ...
        const connections = this.getConnections(this.services.at(serviceIndex));
        const realIndex = connections.controls.indexOf(connection);
        if (realIndex >= 0) {
            connections.removeAt(realIndex);
        }
    }
    getVisibleConnections(service) {
        return this.getConnections(service)
            .controls
            .filter(control => control.get('status')?.value !== 0);
    }
    addOutputField() {
        this.outputFields.push(this.createOutputField());
    }
    ...
    openCurlDialog() {
        this.curlDialogVisible.set(true);
    }
    getAvailableSourceElements(service) {
        const usedElements = this.getConnections(service).controls
            .map(x => x.get('genElement')?.value);
        return this.sourceElementOptions.filter(x => !usedElements.includes(x.value));
    }
    getAvailableTargetElements(service, serviceId) {
        const usedElements = this.getConnections(service).controls
            .map(x => x.get('reqElement')?.value);
        return (this.targetElementOptions[serviceId] || []).filter(x => !usedElements.includes(x.value));
    }
    saveAggregator() {
        debugger;
        ...
        this.services.controls.forEach((service, index) => {
            console.log(`Service ${index} - 401 JSON =>`, service.controls.outputStaticJson401.value);
            console.log(`Service ${index} - 500 JSON =>`, service.controls.outputStaticJson500.value);
        });
        const dto = {
            operationFlag: this.modeFlag === 'U'
                ? 'U'
                : 'I',
            apiId: this.apiId,
            inputCurl: this.curlInput()
                ? this.toBase64Unicode(this.curlInput())
                : null,
            aggregateServiceInfoDtoList: this.services.controls.map(service => {
                const isUpdate = !!service.controls.aggregateServiceId.value;
                console.log('OUTPUTS200 RAW', service.controls.outputs200.getRawValue());
                console.log('OUTPUTS400 RAW', service.controls.outputs400.getRawValue());
                const serviceDto = {
                    operationFlag: isUpdate
                        ? 'U'
                        : 'I',
                    status: service.controls.status.value,
                    aggregatedApiId: service.controls.service.value,
                    aggregatedModuleId: service.controls.module.value,
                    aggregatedPartyId: service.controls.org.value,
                    outputJsonRootNodeName: service.controls
                        .outputJsonRootNodeName
                        .value,
                    clientInfoList: this.form.controls.clientIds.value.map(clientId => {
                        const serviceInfo = this.aggregateDto
                            ?.aggregateServiceInfoDtoList
                            ?.find(x => x.id ===
                            service.controls.aggregateServiceId.value);
                        const existingClient = serviceInfo
                            ?.clientInfoList
                            ?.find(c => c.clientId ===
                            clientId);
                        return {
                            configId: existingClient?.configId ?? null,
                            clientId,
                            status: 1,
                        };
                    }),
                    aggregateServiceMapDtoList: this.getConnections(service)
                        .controls
                        .map(connection => ({
                        inputConfigId: connection.controls.inputConfigId.value,
                        aggregateServiceId: connection.controls.aggregateServiceId.value,
                        status: connection.controls.status.value ?? 1,
                        srcInputEndpointDetailId: connection.controls.genElement.value,
                        destInputEndpointDetailId: connection.controls.reqElement.value,
                        srcInputEndpointDetailTypeId: 2,
                        destInputEndpointDetailTypeId: 2,
                    })),
                    aggregateServiceResponseDtoList: this.buildResponseDto(service)
                        .map(response => ({
                        ...response,
                    })),
                };
        ...
        console.log('CLIENT INFO LIST =>', JSON.stringify(dto.aggregateServiceInfoDtoList.map(x => x.clientInfoList), null, 2));
        console.log('DTO BEFORE SEND =>', JSON.stringify(dto, null, 2));
        /*        this._primengProgressBarService.show();
        ...
        this._primengProgressBarService.show();
        const aggregateId = this.inputAggregatorConfig?.aggregateId;
        const request$ = this.modeFlag === 'U'
            ? this.messagesApiFacadeService.updateAggregate(aggregateId, dto)
            : this.messagesApiFacadeService.registerAggregate(dto);
        request$.subscribe({
            next: () => {
                this._primengProgressBarService.hide();
                this.notifierService.showSuccess({
                    detail: this.modeFlag === 'U'
            ...
            error: error => {
                this._primengProgressBarService.hide();
                console.error(this.modeFlag === 'U'
                    ? 'UPDATE ERROR =>'
                    : 'REGISTER ERROR =>', error);
            },
        });
    }
    buildResponseDto(service) {
        const result = [];
        const buildRows = (outputs, responseStatusCode, responsePartType, isArray) => {
            outputs.controls.forEach((output) => {
                result.push({
                    responseConfigId: output.controls.responseConfigId?.value ?? null,
                    aggregateServiceId: output.controls.aggregateServiceId?.value ?? null,
                    responseStatusCode,
                    responsePartType,
                    responseNodePath: output.controls.sourcePath.value || null,
                    responseNewNodeName: output.controls.key.value || null,
                    isArray: isArray ? 1 : 0,
                    status: output.controls.status?.value ?? 1,
                    outputStaticJson: output.controls.outputStaticJson?.value ?? null,
                });
            });
        };
        // =========================
        // 200
        // =========================
        const responsePartType200 = service.controls.responseMode200.value === 'selected'
            ? 1
            : 0;
        if (responsePartType200 === 1) {
            buildRows(service.controls.outputs200, 200, responsePartType200, service.controls.isArray200.value);
        }
        else {
            result.push({
                responseConfigId: service.controls.responseConfigId200?.value ?? null,
                aggregateServiceId: service.controls.aggregateServiceId?.value ?? null,
        ...
        // 400
        // =========================
        const responsePartType400 = service.controls.responseMode400.value === 'selected'
            ? 1
            : 0;
        if (responsePartType400 === 1) {
            buildRows(service.controls.outputs400, 400, responsePartType400, service.controls.isArray400.value);
        }
        else {
            result.push({
                responseConfigId: service.controls.responseConfigId400?.value ?? null,
                aggregateServiceId: service.controls.aggregateServiceId?.value ?? null,
        ...
        // =========================
        result.push({
            responseConfigId: service.controls.responseConfigId401?.value ?? null,
            aggregateServiceId: service.controls.aggregateServiceId?.value ?? null,
            responseStatusCode: 401,
            responsePartType: 0,
            responseNodePath: service.controls.responseNodePath401?.value ?? null,
            responseNewNodeName: null,
            isArray: 0,
            status: 1,
            outputStaticJson: service.controls.outputStaticJson401.value
                ? this.toBase64Unicode(service.controls.outputStaticJson401.value)
                : null,
        });
        // =========================
        // 500
        // =========================
        result.push({
            responseConfigId: service.controls.responseConfigId500?.value ?? null,
            aggregateServiceId: service.controls.aggregateServiceId?.value ?? null,
            responseStatusCode: 500,
            responsePartType: 0,
            responseNodePath: service.controls.responseNodePath500?.value ?? null,
            responseNewNodeName: null,
            isArray: 0,
            status: 1,
            outputStaticJson: service.controls.outputStaticJson500.value
                ? this.toBase64Unicode(service.controls.outputStaticJson500.value)
                : null,
        });
        return result;
    }
    fromBase64Unicode(value) {
        if (!value?.trim()) {
            return '';
        }
        try {
    ...
    parseCurl(curl) {
        ...
        const methodMatch = curl.match(/(?:--request|-X)\s+([A-Z]+)/i);
        const method = methodMatch?.[1]?.toUpperCase() ?? 'GET';
        const headers = [
            ...curl.matchAll(/(?:--header|-H)\s+(?:'([^']*)'|"([^"]*)")/g),
        ]
            .map(x => x[1] ?? x[2])
            .filter(Boolean);
        const contentType = headers
            .find(h => h.toLowerCase()
            ...
            ?.trim()
            ?.toLowerCase();
        const bodyMatch = curl.match(/(?:-d|--data(?:-raw|-binary|-urlencode)?)\s+'([\s\S]*?)'/m);
        let body = null;
        if (bodyMatch?.[1]) {
...
__decorate([
    Input()
], AggregatorConfigComponent.prototype, "inputAggregatorConfig", void 0);
__decorate([
    Input()
], AggregatorConfigComponent.prototype, "isViewMode", void 0);
__decorate([
    Output()
], AggregatorConfigComponent.prototype, "close", void 0);
AggregatorConfigComponent = __decorate([
    Component({
        selector: 'app-aggregator-config',
        standalone: true,
        imports: [
            CommonModule,
            FormsModule,
...
export { AggregatorConfigComponent };
