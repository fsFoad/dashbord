...
let TimeLimitationComponent = class TimeLimitationComponent {
    route;
    transloco;
    ...
    constructor(route, transloco, messagesApiFacadeService, _primengProgressBarService, apiGatewayService, accessDataSaveService) {
        this.route = route;
    ...
    ngOnInit() {
        ...
        this.messagesApiFacadeService.getlmitbyapiid(this.inputTimeLimitation.apiId).subscribe((a) => {
            ...
            this.timeLimitationList.map((x) => x.status === 1 ? (x.status = true) : (x.status = false));
            for (let k = 0; k < this.timeLimitationList.length; k++) {
                this.timeLimitationList[k] = Object.assign(this.timeLimitationList[k], { row: k + 1 });
            }
        }, (error) => {
            this._primengProgressBarService.hide();
        });
    ...
    onClose(event) {
        ...
        if (event == 'close') {
            ...
            this.messagesApiFacadeService
                ...
                this.timeLimitationList.map((x) => x.status === 1
                    ? (x.status = true)
                    : (x.status = false));
                for (let k = 0; k < this.timeLimitationList.length; k++) {
                    this.timeLimitationList[k] = Object.assign(this.timeLimitationList[k], { row: k + 1 });
...
__decorate([
    Output()
], TimeLimitationComponent.prototype, "close", void 0);
__decorate([
    Input()
], TimeLimitationComponent.prototype, "inputTimeLimitation", void 0);
TimeLimitationComponent = __decorate([
    Component({
...
export { TimeLimitationComponent };
