...
let ApiHubAttachmentComponent = class ApiHubAttachmentComponent {
    ...
    tooltipBack = 'به علت اتصال سرویس به هاب امکان بازگشت به مراحل قبل وجود ندارد!';
    ...
    messagesList400Temp = [];
    messagesList500Temp = [];
    constructor(messagesApiFacadeService, _primengProgressBarService, confirmationService, transloco, notifierService) {
    ...
    checkDontCareAndAddEleman(e) {
        ...
        for (let i = 0; i < this.paramsCustomList.length; i++) {
            if (this.paramsCustomList[i]?.detailType == undefined ||
    ...
    onContinuation() {
        if (this.selectedHubList != undefined) {
            ...
            if (this.selectedHubList.messageId2X != null &&
            ...
            if (this.selectedHubList.messageId4X != null &&
            ...
            if (this.selectedHubList.messageId5X != null &&
            ...
            window.localStorage.setItem('hubObj', JSON.stringify(this.selectedHubList));
            // this.commandTypeId = this.selectedHubList.commandTypeId.toString()
        }
        else {
            this.notifierService.showError({
 ...
 // Convert the grouped map back to an array
   ...
   // Convert the grouped map back to an array
                        ...
                        if (this.paramsList.length > 0) {
                            for (let i = 0; i < this.paramsList.length; i++) {
                                ...
                                if (this.paramsList[i]?.paramValue != undefined) {
                                    if (this.paramsList[i]?.paramValue != null &&
    ...
    deleteParam(param) {
        ...
        if (index > -1) {
            ...
            for (let k = 0; k < this.paramsStaticList.length; k++) {
                this.paramsStaticList[k] = Object.assign(this.paramsStaticList[k], { row: k + 1 });
            }
    ...
    deleteCusParam(param) {
        const index = this.paramsCustomList.findIndex((obj) => obj.row === param.row);
        if (index > -1) {
    ...
    deleteSpParam(param) {
        ...
        const index2 = this.staticAndNonStaticList.findIndex((obj) => obj.row === param.row);
        if (index > -1) {
            // حذف شیء از spParamsList
            this.spParamsList = this.spParamsList.filter((obj) => obj.row !== param.row);
        ...
        if (index2 > -1) {
            this.staticAndNonStaticList = this.staticAndNonStaticList.filter((obj) => obj.row !== param.row);
    ...
    testSp() {
        ...
        this.spParamsList.forEach((spParamsItem) => {
            ...
            const testSpDto1 = {
                name: spParamsItem.name == null || spParamsItem.name == undefined
                    ? ''
                    : spParamsItem.name,
                aliasOutputParamName: spParamsItem.aliasOutputParamName,
                testValue: spParamsItem.paramValue == undefined
                    ? null
    ...
    addUniqueItems(arr1, arr2, keys) {
        arr1.forEach((item1) => {
            const isDuplicate = arr2.some((item2) => this.areObjectsEqual(item1, item2, keys));
            if (isDuplicate) {
    ...
    mergeDuplicateParams(paramsList) {
        const mergedParams = {};
        ...
        paramsList.forEach((param) => {
            ...
            console.error('Merged params so far:', JSON.stringify(mergedParams));
        });
    ...
    mergeData(list1, list2) {
        ...
        list2.forEach((item2) => {
            ...
            const match = list1.find((item1) => item1.paramName === item2.paramName);
            // اگر تطابق پیدا شد
            if (match) {
                const combinedItem = {
    ...
    matchElemanLists(list1, list2) {
        ...
        list1.forEach((item) => {
            if (!list2.some((element) => element.name === item.paramName)) {
                ...
                console.error('Adding unmatched item from list1:', JSON.stringify(newItem));
    ...
    magicCusMethod() {
        ...
        tempList.forEach((item) => {
            ...
            const existingItem = result.find((obj) => (obj.name || obj.paramName || item.inputName) === key);
            if (existingItem) {
                ...
                if (item.paramName.includes(':')) {
                    ...
                    this.matches.forEach((paramName) => {
                        ...
                        if (!existingParam) {
                            ...
                            console.error('this.paramsCustomList', JSON.stringify(this.paramsCustomList));
                        }
                    });
                }
                else {
        ...
        for (let k = 0; k < this.paramsCustomList.length; k++) {
            if (+this.paramsCustomList[k]?.endpointDetailId != 0) {
                this.nonStaticParam.push({
                    ...
                    endpointDetailId: +this.paramsCustomList[k]?.endpointDetailId,
                });
            }
        }
 ...
 */
    processData(data, flag) {
        ...
        const result = Object.values(grouped).map((group) => {
            ...
            if (flag === 2) {
                ...
                mergedObject = {
                    ...
                    dataType: typeof mergedObject.dataType === 'number'
                        ? mergedObject.dataType
                        : parseInt(mergedObject.dataType, 10),
                    operationTypeId: typeof mergedObject.operationTypeId === 'number'
                        ? mergedObject.operationTypeId
                        : parseInt(mergedObject.operationTypeId, 10),
                    endpointDetailId: typeof mergedObject.endpointDetailId === 'number'
        ...
        if (flag === 2) {
            const uniqueData = data.filter((item, index, array) => index ===
                array.findIndex((obj) => obj.endpointDetailId === item.endpointDetailId));
    ...
    confirmDialog(type, obj) {
        ...
        if (type == 'sp') {
            ...
            this.confirmationService.confirm({
                message: 'پاکسازی params موجب میگردد اطلاعات جاری پاک شوند.آیا از پاکسازی  params اطمینان دارید؟',
                accept: () => {
        ...
        if (type == 'param') {
            this.headerConfirm = 'clear params';
            this.confirmationService.confirm({
                message: 'load params api موجب میگردد اطلاعات جاری پاک شوند.آیا از پاکسازی  params اطمینان دارید؟',
                accept: () => {
                    //  this.staticAndNonStaticList = []
                    ...
                    this._primengProgressBarService.show();
                    this.messagesApiFacadeService.endpointdetailByApi(this.apiId, 0, 10000).subscribe((response) => {
                        this._primengProgressBarService.hide();
                        debugger;
                        let tempDetailList;
                        this.paramsList = [];
                        ...
                        tempDetailList.map((x) => x.status === 1
                            ? (x.status = true)
                            : (x.status = false));
                        const conter = 0;
                                    ...
                                    if (this.type != undefined) {
                                        if (this.type == 2) {
                                            if (tempDetailList[i]
                                                ...
                                                if (tempDetailList[i]
                                                    ...
                                                    tempDetailList[i] =
                                                        Object.assign(tempDetailList[i], {
                                                            paramName: tempDetailList[i]
                                                                .ouputName,
                                                            paramValue: tempDetailList[i]
                                                                .outputValue,
                                                        });
                                                }
                                                else if (tempDetailList[i]
                                                    .actionType == 2) {
                                                    tempDetailList[i] =
                                                        Object.assign(tempDetailList[i], {
                                                            paramName: tempDetailList[i]
                                                ...
                                                if (!isNaN(tempDetailList[i]?.outputValue)) {
                                                    tempDetailList[i] =
                                                        Object.assign(tempDetailList[i], {
                                                            dataType: 1,
                                                        });
                                                ...
                                                if (this.paramsList
                                                    ...
                                                    this.paramsList.push(tempDetailList[i]);
                                                    this.paramsStaticList.push(tempDetailList[i]);
                                                }
                                                else {
                                                    tempDetailList[i] =
                                                        Object.assign(tempDetailList[i], {
                        ...
                        if (this.tempAddParam.length > 0) {
                            this.paramsList = this.mergeParamLists(this.tempAddParam, this.paramsList);
                            this.paramsStaticList =
                                this.mergeParamLists(this.paramsList, this.paramsStaticList);
                            this.tempAddParam = this.paramsList;
                            /*   const updatedArray2 = this.tempAddParam.map(item2 => {
                           const matchingItem = this.paramsList.find(item1 => {
                                   item1.endpointDetailId === item2.endpointDetailId
                           ...
                           return item2;
                            ...
                            // this.paramsList = []
                            for (let k = 0; k < this.paramsList.length; k++) {
                                this.paramsList[k] = Object.assign(this.paramsList[k], { row: k + 1 });
                            }
                            // Create a map to group by paramName
                            const grouped = new Map();
                            ...
                            this.paramsStaticList.forEach((item) => {
                                if (staticGrouped.has(item.paramName)) {
                                    // If paramName already exists, combine data
                                    const existingItem = staticGrouped.get(item.paramName);
                                    // Merge the two objects (you can modify this merging erroric based on your requirements)
                                    for (const key in item) {
                                        if (key !== 'paramName' &&
                                            key !== 'row') {
                            ...
                            const mergedArray = Array.from(grouped.values());
                            const staticMergedArray = Array.from(staticGrouped.values());
                            for (let k = 0; k < mergedArray.length; k++) {
                                mergedArray[k] = Object.assign(mergedArray[k], { row: k + 1 });
                            }
                            this.paramsList = mergedArray;
        ...
        if (type == 'relodCus') {
            ...
            this.headerConfirm = 'clear params';
            this.confirmationService.confirm({
                message: 'validate query  موجب میگردد اطلاعات جاری پاک شوند.آیا از پاکسازی params اطمینان داردید؟',
                accept: () => {
                    ...
                    for (let i = 0; i < this.cusParamList.length; i++) {
                        this.cusParamList[i] = Object.assign(this.cusParamList[i], { actionType: null });
                    }
                    this._primengProgressBarService.show();
                    this.messagesApiFacadeService
...
*/ this.paramsList = [];
                        ...
                        tempDetailList = this.paramsList;
                        this.filterList = [];
                        tempDetailList.forEach((x) => (x.status = x.status === 1));
                        const conter = 0;
                        for (let i = 0; i < tempDetailList.length; i++) {
                            const counter = 0;
                            if (tempDetailList[i]
                                    ...
                                    if (this.type != undefined) {
                                        if (this.type == 2) {
                                            if (tempDetailList[i]
                                                ...
                                                if (tempDetailList[i]
                                                    .actionType == 1) {
                                                    this.filterList.push(Object.assign(tempDetailList[i], {
                                                        paramName: tempDetailList[i]
                                                            .ouputName,
                                                        paramValue: tempDetailList[i]
                                                            .outputValue,
                        ...
                        for (let i = 0; i < this.filterList.length; i++) {
                            if (!isNaN(this.filterList[i]?.outputValue)) {
                                this.filterList[i] = Object.assign(this.filterList[i], { dataType: 1 });
                            }
                            else {
                                this.filterList[i] = Object.assign(this.filterList[i], { dataType: 0 });
                            }
                        }
                        // this.paramsCustomList.forEach(n=>{
                        ...
                        tempList.forEach((item) => {
                            ...
                            const existingItem = result.find((obj) => (obj.name ||
                                obj.paramName ||
                                item.inputName) === key);
                            if (existingItem) {
                                // اگر وجود داشت، مقادیر را ادغام می‌کنیم
                        ...
                        if (this.tempCusAddParam.length > 0) {
                            ...
                            updatedArray2.forEach((item) => {
                                ...
                                if (item.paramName.includes('=:')) {
                                    // اگر داده خاصی با فرمت =: پیدا شد، آن را به paramsCustomList اضافه کنیم
                                    this.matches.forEach((paramName) => {
                                        const existingParam = this.paramsCustomList.find((item) => item.paramName ===
                                            paramName);
                                        if (!existingParam) {
                                            // اگر پارامتر وجود نداشت، آن را اضافه می‌کنیم
                                            this.paramsCustomList.push({
                                            ...
                                            this.paramsCustomList =
                                                this.mergeDuplicateParams(this
                                                    .paramsCustomList);
                                            console.error('this.paramsCustomList', JSON.stringify(this
                                                .paramsCustomList));
                                        }
                                    });
                                }
                            ...
                            this.paramsCustomList =
                                this.mergeDuplicateParams(this.paramsCustomList);
                            this.paramsCustomList.forEach((item) => {
                                this.matches = this.matches.map((item) => item.trim());
                                if (this.matches.includes(item.paramName)) {
                                    item.matched = 1; // اگر در matches وجود داشت
                        ...
                        for (let k = 0; k < this.paramsCustomList.length; k++) {
                            if (+this.paramsCustomList[k]
                                .endpointDetailId != 0) {
                                this.nonStaticParam.push({
                                    dataType: +this.paramsCustomList[k]
                                        .dataType,
                                    paramType: 0,
                                    aliasOutputParamName: null,
                                    apiDataHubParseOutputParamDomain: null,
                                    actionType: +this.paramsCustomList[k]
                                        .actionType,
                                    operationTypeId: +this.paramsCustomList[k]
                                        .operationTypeId,
                                    endpointDetailId: +this.paramsCustomList[k]
                                        .endpointDetailId,
                                });
                            }
                        }
                ...
                accept: () => {
                    ...
                    this.messagesApiFacadeService.registerEndpointdetail(levelId, recordId, registerTemp).subscribe((a) => {
                        ...
                        this.messagesApiFacadeService
                                        ...
                                        if (this.type != undefined) {
                                            if (this.type == 2) {
                                                if (tempDetailList[i]?.detailType == 3) {
                                                    ...
                                                    else if (tempDetailList[i].actionType == 2) {
                                                        tempDetailList[i] =
                                                            Object.assign(tempDetailList[i], {
                                                                paramName: tempDetailList[i]
                                                                    ?.inputName,
                                                    ...
                                                    else if (tempDetailList[i].actionType == 3) {
                                                        tempDetailList[i] =
                                                            Object.assign(tempDetailList[i], {
                                                                paramName: tempDetailList[i]
                                                                    ?.inputName,
                                                    ...
                                                    if (!isNaN(tempDetailList[i]
                                                        .outputValue)) {
                                                        tempDetailList[i] =
                                                            Object.assign(tempDetailList[i], { dataType: 1 });
                                                    }
                                                    else {
                                                        tempDetailList[i] =
                                                            Object.assign(tempDetailList[i], { dataType: 0 });
                                                    }
                                                    if (this.paramsList.length > 0) {
                                                        tempDetailList[i] =
                                                            Object.assign(tempDetailList[i], { dataType: 0 }, { disabled: true });
                                                        this.paramsList.push(tempDetailList[i]);
                                                        this.paramsStaticList.push(tempDetailList[i]);
                                                    }
                                                    else {
                                                ...
                                                if (tempDetailList[i]?.detailType == 2) {
                                                    ...
                                                    if (this.paramsList.length > 0) {
                                                        tempDetailList[i] =
                                                            Object.assign(tempDetailList[i], { disabled: true });
                                                        this.paramsList.push(tempDetailList[i]);
                                                        this.paramsStaticList.push(tempDetailList[i]);
                                                    }
                                                    else {
                                                        ...
                                                        this.paramsList.push(tempDetailList[i]);
                                                        this.paramsStaticList.push(tempDetailList[i]);
                                                        console.error('this.paramsList', this.paramsList);
                                                        console.log('this.paramsStaticList', this.paramsStaticList);
                                                    }
                                                }
                                            }
                                        }
                            ...
                            if (this.tempAddParam.length > 0) {
                                const updatedArray2 = this.tempAddParam.map((item2) => {
                                    const matchingItem = this.paramsList.find((item1) => item1.inputName
                                        .trim()
                                        .toLowerCase() ===
                                        item2.paramName
                                            .trim()
                                    ...
                                    if (matchingItem) {
                                        return {
                                            ...
                                            operationTypeId: matchingItem.operationTypeId ??
                                                item2.operationTypeId,
                                            disabled: true,
                                            endpointDetailId: matchingItem.endpointDetailId,
                                            detailType: 1,
                                ...
                                this.paramsList = this.processData(updatedArray2, 1);
                                console.error(this.paramsList);
                                this.paramsStaticList = this.processData(this.paramsList, 2);
                            }
                            for (let k = 0; k < this.paramsList.length; k++) {
        ...
        if (type == 'ignoreParams') {
            ...
            this.confirmationService.confirm({
                message: 'عدم افزودن المان سرویس موجب فراخوانی کل اطلاعات در دیتاهاب می گردد، آیا از نادیده گرفتن پارامتر ها اطمینان دارید؟',
                accept: () => {
                    let counter = 0;
                    for (let i = 0; i < this.paramsList.length; i++) {
                        if (this.paramsList[i]?.paramValue == undefined) {
    ...
    mergeParamLists(list1, list2) {
        const mergedList = [];
        const matchedIndices = new Set(); // برای ذخیره آیتم‌های تطابق‌یافته از لیست دوم
        // پردازش لیست اول
        list1.forEach((item1) => {
            let isMatched = false;
    ...
    loadNonStaticParams() {
        ...
        if (this.paramsList.length > 0) {
            ...
            (response) => {
                ...
                tempDetailList.map((x) => x.status === 1 ? (x.status = true) : (x.status = false));
                const conter = 0;
                for (let i = 0; i < tempDetailList.length; i++) {
                    const counter = 0;
                    if (tempDetailList[i]?.isSystemEndpointDetail == 1) {
                        ...
                        if (tempDetailList[i]?.status == 1 || tempDetailList[i]?.status) {
                            if (this.type != undefined) {
                                if (this.type == 2) {
                                    if (tempDetailList[i]?.detailType == 3) {
                                        if (tempDetailList[i].actionType == 1) {
                                            tempDetailList[i] = Object.assign(tempDetailList[i], {
                                                paramName: tempDetailList[i]?.ouputName,
                                                paramValue: tempDetailList[i].outputValue,
                                            });
                                        }
                                        else if (tempDetailList[i].actionType == 2) {
                                            tempDetailList[i] = Object.assign(tempDetailList[i], {
                                                paramName: tempDetailList[i]?.inputName,
                                                paramValue: tempDetailList[i].outputValue,
                                            });
                                        }
                                        else if (tempDetailList[i].actionType == 3) {
                                        ...
                                        if (!isNaN(tempDetailList[i].outputValue)) {
                                            tempDetailList[i] = Object.assign(tempDetailList[i], { dataType: 1 });
                                        }
                                        else {
                                            tempDetailList[i] = Object.assign(tempDetailList[i], { dataType: 0 });
                                        }
                                        if (this.paramsList.length > 0) {
                                            tempDetailList[i] = Object.assign(tempDetailList[i], { dataType: 0 }, { disabled: true });
                                            this.paramsList.push(tempDetailList[i]);
                                            this.paramsStaticList.push(tempDetailList[i]);
                                        }
                                        else {
                                    ...
                                    if (tempDetailList[i]?.detailType == 2) {
                                        ...
                                        if (this.paramsList.length > 0) {
                                            tempDetailList[i] = Object.assign(tempDetailList[i], { disabled: true });
                                            this.paramsList.push(tempDetailList[i]);
                                            this.paramsStaticList.push(tempDetailList[i]);
                                        }
                                        else {
            ...
            this.messagesApiFacadeService.endpointdetailByApi(this.apiId, 0, 10000).subscribe((response) => {
                ...
                for (let i = 0; i < tempDetailList.length; i++) {
                    ...
                    this.spParamsList.push({
                        ...
                        operationTypeId: item?.operationTypeId ?? 0,
                        row: 1,
                        status: true,
                        paramType: '0',
                        aliasOutputParamName: '',
                ...
                for (let k = 0; k < this.spParamsList.length; k++) {
                    if (this.spParamsList[k]?.paramType == '0') {
                        counterI++;
                        this.spParamsList[k] = Object.assign(this.spParamsList[k], { row: k + 1 }, { name: 'I' + counterI });
                    }
                }
            }, (error) => {
                this._primengProgressBarService.hide();
    ...
    fakeParamRegister() {
        if (this.validationParam()) {
            ...
            this.param = '';
            if (this.paramsList.length > 0) {
                if (this.checkDuplicate(this.paramsList, {
    ...
    addObject(list) {
        ...
        if (this.spParamsList.length == 0) {
            ...
            this.staticAndNonStaticList.push({
                ...
                paramType: +this.paramType,
                operationTypeId: +this.operationTypeId,
                aliasOutputParamName: this.aliasOutputParamName == undefined ||
                    this.aliasOutputParamName == ''
                    ? null
                    : this.aliasOutputParamName,
                apiDataHubParseOutputParamDomain: this.successValue == undefined ||
                    this.successValue == '' ||
            ...
            if (namePrefix) {
                if (namePrefix == 'O') {
                    const currentIndexes = list
                        ...
                        .map((item) => parseInt(item.name.substring(namePrefix.length), 10))
        ...
        for (let k = 0; k < this.staticAndNonStaticList.length; k++) {
            if ('row' in this.staticAndNonStaticList) {
                ...
                this.staticAndNonStaticList[k] = Object.assign(this.staticAndNonStaticList[k], { row: k + 1 });
            }
        }
    }
    fakeParamSpRegister() {
    ...
    fakeMapRegister() {
        if (this.validationMap()) {
            if (this.mapList.length > 0) {
                if (this.mapcheckDuplicate(this.mapList, {
                    inputName: this.inputName,
                    outputName: this.outputName,
                    dataType: this.dataType,
                    isEcrypt: this.isEcrypt == true
                        ? (this.isEcrypt = true)
                        : (this.isEcrypt = false),
                })) {
                    this.mapList.push({
   ...
   */
    ...
    extractWhereConditions(sql) {
        const conditionRegex = /(WHERE|AND)\s+([A-Z0-9._]+)\s*=\s*([A-Z0-9._]+)/gi;
        let match;
        const conditions = [];
        ...
        if (this.customQuery) {
            if (this.paramsCustomList.length > 0) {
                ...
                (response) => {
                            ...
                            if (tempDetailList[i]?.status == 1 || tempDetailList[i]?.status) {
                                if (this.type != undefined) {
                                    if (this.type == 2) {
                                        if (tempDetailList[i].detailType == 3) {
                                            if (tempDetailList[i].actionType == 1) {
                                                filterList.push(Object.assign(tempDetailList[i], {
                                                    paramName: tempDetailList[i].ouputName,
                                                    paramValue: tempDetailList[i].outputValue,
                                                }));
                                            }
                    ...
                    let tempList;
                    tempList = this.matchElemanLists(filterList, this.cusParamList);
                    const items = tempList;
                    const paramNamesSet = new Set();
                    for (const item of items) {
                        if (!paramNamesSet.has(item.name == undefined
                            ? item.paramName
                            : item.name)) {
                            paramNamesSet.add(item.name == undefined
                                ? item.paramName
                                : item.name);
                            this.matches.forEach((paramName) => {
                                const existingParam = this.paramsCustomList.find((item) => item.paramName === paramName);
                    ...
                    for (let k = 0; k < this.paramsCustomList.length; k++) {
                        this.nonStaticParam.push({
                            dataType: Number(this.paramsCustomList[k]?.dataType),
                            operationTypeId: +this.paramsCustomList[k]
                                .operationTypeId,
                            paramType: 0,
                            aliasOutputParamName: null,
                    ...
                    this.paramsCustomList = this.mergeData(this.paramsCustomList, filterList);
                    this.matches = this.matches.map((item) => item.trim());
                    this.paramsCustomList.forEach((item) => {
                        if (this.matches.includes(item.paramName)) {
                            item.matched = 1;
                    ...
                    debugger;
                    this.paramsCustomList.forEach((item) => {
                        this.matches[0] = this.matches[0]?.replace(/=/g, '');
                        this.matches = this.matches.map((item) => item?.trim());
                        if (this.matches.includes(item.paramName)) {
                            item.matched = 1;
                        }
    ...
    testQuery() {
        ...
        if (tempObjHub != undefined && tempObjHub != null) {
            if (this.hubId != null) {
                ...
                this._primengProgressBarService.show();
                this.messagesApiFacadeService
                    .testconnectionandquery(this.hubId, this.apiDataHubId, this.tempQuery)
                    .subscribe((b) => {
                    this._primengProgressBarService.hide();
                    this.resultTestQueryFlag = true;
                    this.querySuccessFlag = true;
    ...
    replaceParams(query, params) {
        ...
        params.forEach((param) => {
            ...
            if (param.dataType == '0') {
                updatedQuery = updatedQuery.replace(regex, param.paramValue !== null ? `'${param.paramValue}'` : 'NULL');
            }
            else {
                updatedQuery = updatedQuery.replace(regex, param.paramValue !== null ? param.paramValue : 'NULL');
            }
        ...
        if (tempObjHub != undefined && tempObjHub != null) {
            if (this.hubId != null) {
                ...
                this.downloadResultSet
                    ...
                    : (this.hubObj.downloadResultSet = 0);
                if (this.commandTypeId == '3') {
                    strReplace = this.replaceParams(newSqlQuery, this.paramsCustomList);
                }
                this._primengProgressBarService.show();
                this.messagesApiFacadeService
                    .testconnectionandquery(this.hubId, this.apiDataHubId, strReplace)
                    .subscribe((b) => {
                    this._primengProgressBarService.hide();
                    this.resultTestQueryFlag = true;
                    this.querySuccessFlag = true;
    ...
    updateHighlight() {
        this.highlightedQuery = this.customQuery
            .replace(/where\b(.*?)= *:([\w.]+)/gi, (match, beforeWhere, afterWhere) => {
    ...
    searchHubAttachList() {
        ...
        this._primengProgressBarService.show();
        this.messagesApiFacadeService
            .searchhub(this.pageno, this.pagesize, this.dbnameSearch, this.ipSearch, this.titleSearch)
            ...
            this.hubAttachList.map((x) => x.status === 1 ? (x.status = true) : (x.status = false));
                        ...
                        this.messagesApiFacadeService
                            ...
                            for (let k = 0; k < this.paramsStaticList.length; k++) {
                                this.paramsStaticList[k] =
                                    Object.assign(this.paramsStaticList[k], {
                                        apiHubId: this.apiDataHubId,
                                    });
                            }
                            for (let k = 0; k < this.paramsStaticList.length; k++) {
                                this.paramsStaticList[k] =
                                    Object.assign(this.paramsStaticList[k], { actionType: null });
                            }
                            /* let counter = 0
                     ...
                     if (this.paramsStaticList[r]?.paramType != '0') {
                         ...
                         this.paramsStaticList[r]?.endpintdetailId = counter
                            ...
                            if (this.paramsStaticList.length > 0) {
                                if (this.tempAddParam.length > 0) {
                                    this.paramsList =
                                        this.mergeParamLists(this.tempAddParam, this.paramsList);
                                    this.paramsStaticList =
                                        this.mergeParamLists(this.paramsList, this.paramsStaticList);
                                    this.tempAddParam =
                                        this.paramsList;
                                    for (let k = 0; k < this.paramsList.length; k++) {
                                        this.paramsList[k] =
                                            Object.assign(this.paramsList[k], { row: k + 1 });
                                    }
                                    // Create a map to group by paramName
                                    const grouped = new Map();
                                    const staticGrouped = new Map();
                                    ...
                                    this.paramsStaticList.forEach((item) => {
                                        if (staticGrouped.has(item.paramName)) {
                                            ...
                                            // If paramName doesn't exist, add the item to the map
                                            staticGrouped.set(item.paramName, { ...item });
                                        }
                                    });
                                    const mergedArray = Array.from(grouped.values());
                                    const staticMergedArray = Array.from(staticGrouped.values());
                                    for (let k = 0; k < mergedArray.length; k++) {
                                        mergedArray[k] =
                                            Object.assign(mergedArray[k], { row: k + 1 });
                                    }
                                    this.paramsList = mergedArray;
                                    this.paramsStaticList =
                                        staticMergedArray;
                                ...
                                let tempArray = this.paramsStaticList;
                                tempArray = tempArray.map((obj) => {
                                    ...
                                    newObj.operationTypeId = Number(newObj.operationTypeId);
                                    newObj.actionType = 0;
                                    newObj.paramType = 0;
                                    newObj.dataType =
                                        +newObj.dataType;
                                ...
                                this.messagesApiFacadeService
                                    ...
                                    if (mappingListReg.length >
                                        0) {
                                        for (let k = 0; k <
                                            mappingListReg.length; k++) {
                                            mappingListReg[k] =
                                                Object.assign(mappingListReg[k], {
                                                    apiHubId: this
                                                        .apiDataHubId,
                                                });
                                        }
                                        this._primengProgressBarService.show();
                                        this.messagesApiFacadeService
                                            .registerApidatahubmapresult(mappingListReg)
                                            .subscribe((h) => {
                                            this._primengProgressBarService.hide();
                   ...
                   for (let r = 0; r < this.paramsStaticList.length; r++) {
                       ...
                       if (this.paramsStaticList[r]?.paramType != '0') {
                           ...
                           this.paramsStaticList[r]?.endpintdetailId = counter
                                ...
                                for (let k = 0; k < mappingListReg.length; k++) {
                                    mappingListReg[k] =
                                        Object.assign(mappingListReg[k], {
                                            apiHubId: this
                                                .apiDataHubId,
                        ...
                        this.messagesApiFacadeService
                            ...
                            this._primengProgressBarService.hide();
                            let counter = 0;
                            if (this.staticAndNonStaticList.length >
                                0) {
                                for (let q = 0; q <
                                    this.staticAndNonStaticList
                                        .length; q++) {
                                    if (this.staticAndNonStaticList[q]?.paramType != '0') {
                                        counter--;
                            ...
                            this.staticAndNonStaticList =
                                this.staticAndNonStaticList.map((obj) => {
                                    delete obj.row;
                                    return obj;
                                });
                            for (let k = 0; k <
                                this.staticAndNonStaticList.length; k++) {
                                this.staticAndNonStaticList[k] =
                                    Object.assign(this.staticAndNonStaticList[k], {
                                        apiHubId: this.apiDataHubId,
                                    });
                            }
                            //برای don't care
                            for (let k = 0; k <
                                this.staticAndNonStaticList.length; k++) {
                                this.staticAndNonStaticList[k] =
                                    Object.assign(this.staticAndNonStaticList[k], { actionType: 0 });
                            }
                            if (this.staticAndNonStaticList.length >
                                0) {
                                this._primengProgressBarService.show();
                                this.messagesApiFacadeService
                                    .registerApihubparam(this.staticAndNonStaticList)
                                    .subscribe((p) => {
                                    this._primengProgressBarService.hide();
                                    let mappingListReg = [];
                                    for (let k = 0; k <
                                ...
                                mappingListReg = mappingListReg.map((obj) => {
                                    delete obj.row;
                                    return obj;
                                });
                                for (const item of this.mapList) {
                        ...
                        this.messagesApiFacadeService
                            ...
                            for (let k = 0; k < this.nonStaticParam.length; k++) {
                                this.nonStaticParam[k] =
                                    Object.assign(this.nonStaticParam[k], {
                                        apiHubId: this.apiDataHubId,
                                    });
                                if (this.nonStaticParam[k]
                                    .actionType == null ||
                                    this.nonStaticParam[k]
                                        .actionType == 3) {
                            ...
                            if (this.nonStaticParam.length > 0) {
                                this._primengProgressBarService.show();
                                this.messagesApiFacadeService
                                    .registerApihubparam(this.nonStaticParam)
                                    .subscribe((p) => {
                                    this._primengProgressBarService.hide();
                                    const hubObj = new HubDto();
                                    hubObj.hubId =
...
__decorate([
    Input()
], ApiHubAttachmentComponent.prototype, "inputAttachApiHub", void 0);
__decorate([
    Output()
], ApiHubAttachmentComponent.prototype, "close", void 0);
ApiHubAttachmentComponent = __decorate([
    Component({
...
export { ApiHubAttachmentComponent };
