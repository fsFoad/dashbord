...
import { HeaderEndpointRegisterComponent, } from '../../../endpoint-management/header-endpoint-management/header-endpoint-register/header-endpoint-register.component';
import { CacheDto } from '../../../../../models/cache.Dto';
import { CastToDateTimePipe } from '../../../../../../shared/pipes/cast-to-date-time.pipe';
let CacheListComponent = class CacheListComponent {
    transloco;
    notifierService;
    ...
    constructor(transloco, notifierService, _primengProgressBarService, cd, messagesApiFacadeService, apiGatewayService) {
    ...
    getEndpointLabel(apiId, endpointDetailId) {
    ...
    loadEndpointOptionsFor(apiId, forceReload = false, endpointDetailId) {
        this.apiIdForAddElement = apiId;
        if (this.endpointOptionsMap[apiId] && !forceReload)
            return; // فقط اگر لود نشده یا نیاز به رفرش نیست
        this.messagesApiFacadeService.endpointdetailByApi(Number(apiId), 0, 1000).subscribe((res) => {
            this._primengProgressBarService.hide();
            const filtered = (Array.isArray(res) ? res : [res]).filter(item => item?.detailType === 3 && item?.actionType === 3);
    ...
    formatDurationArray(data) {
        const LRM = '\u200E';
        return data.map(item => {
            const parts = [];
            if (item.year)
                parts.push(`${LRM}${item.year}Y`);
            if (item.month)
                parts.push(`${LRM}${item.month}M`);
            if (item.day)
                parts.push(`${LRM}${item.day}D`);
            if (item.hour)
                parts.push(`${LRM}${item.hour}h`);
            if (item.minute)
                parts.push(`${LRM}${item.minute}m`);
            return parts.join(' ');
        }).join('\n');
    }
    ngOnInit() {
            ...
            this.messagesApiFacadeService.apicacheStatus(cache?.apicacheid, 1).subscribe(x => {
                ...
                this.messagesApiFacadeService.fetchApicacheList(requestBody).subscribe(res => {
                    ...
                    this.cacheList.forEach(item => {
                        if (!item.endpointDetailId || !item.inputName)
                            return;
                        if (!endpointOptionsMap[item.apiId])
                            endpointOptionsMap[item.apiId] = [];
    ...
    mapSavedData(input) {
        return input.map((item, idx) => ({
            ...
            endpointDetailId: (item.apiCacheEndpointDetailDomainList &&
                item.apiCacheEndpointDetailDomainList.length > 0) ? item.apiCacheEndpointDetailDomainList[0].endpointDetailId : (item.endpointDetailId ?? null),
            inputName: (item.apiCacheEndpointDetailDomainList &&
                item.apiCacheEndpointDetailDomainList.length > 0) ? item.apiCacheEndpointDetailDomainList[0].endpointDetailInputName : (item.endpointDetailInputName ?? null),
            ...
            expiryDate: ((item.expireYY ? `${item.expireYY}Y ` : '') +
                (item.expireMM ? `${item.expireMM}M ` : '') +
                (item.expireDD ? `${item.expireDD}D` : '')).trim() || null,
            savedFlag: true,
        }));
    }
    saveCache() {
            ...
            this.messagesApiFacadeService.apicacheRegister(this.moduleId, mapped).subscribe(res => {
                ...
                this.messagesApiFacadeService.fetchApicacheList(requestBody).subscribe(res2 => {
                    ...
                    for (let k = 0; k < this.cacheList.length; k++) {
                        this.cacheList[k] = Object.assign(this.cacheList[k], { row: k + 1 });
                    }
    ...
    refreshDataAndPreserveSavedItems(serverData, saved) {
        debugger;
    ...
    refreshDataAndPreserveUnsavedItems(serverData, unsaved) {
        debugger;
...
__decorate([
    Input()
], CacheListComponent.prototype, "inputApiCache", void 0);
__decorate([
    Output()
], CacheListComponent.prototype, "exit", void 0);
CacheListComponent = __decorate([
    Component({
