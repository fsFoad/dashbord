...
let WageRegisterComponent = class WageRegisterComponent {
    route;
    ...
    inputFloat = ApiGatewayConstants.cust_float;
    rows = 10;
    first = 0;
    wageList = [];
    apiList = [];
    ...
    detailsBreadObject = [];
    constructor(route, commonValidationsService, notifierService, transloco, apiGatewayService, _primengProgressBarService, messagesApiFacadeService) {
        this.route = route;
    ...
    ngOnInit() {
        ...
        if (this.wageObj.feeTitle != undefined &&
            ...
            this.messagesApiFacadeService.findbyfeeheaderid(this.wageObj.feeId).subscribe((u) => {
                ...
                for (let k = 0; k < this.wageDetailList.length; k++) {
                    this.wageDetailList[k] = Object.assign(this.wageDetailList[k], { feeTypeId: u.feeTypeId, title: u.title });
                }
        ...
        this.scrollTop();
        if (this.wageObj.partyTitle != null &&
            ...
            if (this.wageObj.clientTitle != null &&
    ...
    disableMoreThanOneAddForFixedTypeWage() {
    ...
    onPercentageOrFixedAmount(event) {
        if (event.value === 1) {
            this.isPercentage = false;
            this.calculationValue = 0;
        }
    ...
    addWageDetail() {
        if (this.validationRegister(false)) {
            if (this.feeTypeId == '4' || this.feeTypeId == '3') {
                this._primengProgressBarService.show();
                this.messagesApiFacadeService.apibyid(this.apiId).subscribe((c) => {
                    this._primengProgressBarService.hide();
                    if (c.feeFieldPath == null ||
                        ...
                        for (let k = 0; k < this.wageDetailList.length; k++) {
                            if ('row' in this.wageDetailList) {
                                ...
                                this.wageDetailList[k] = Object.assign(this.wageDetailList[k], { row: k + 1 });
                            }
                            // this.wageType=null
                            // this.feeTitle=null
                            this.isPercentage = false;
                ...
                if ((this.feeTypeId == '1' || this.feeTypeId == '3') &&
                    this.wageDetailList.length == 1) {
                    this.sadehDonFlag = true;
                }
                else {
    ...
    validateReg() {
        if (!this.commonValidationsService.stringNotNullChecker(this.feeTitle)) {
            this.notifierService.showError({
        ...
        else if (!this.commonValidationsService.stringNotNullChecker(this.feeTypeId)) {
            this.notifierService.showError({
    ...
    onRegister() {
        if (this.validateReg()) {
            const headerWageDto = new HeaderWageDto();
            const feeDetailDomainsDto = new FeeDetailDomainsDto();
            ...
            if (this.commonValidationsService.stringNotNullChecker(this.wageObj.partyId)) {
                this.partyId = this.wageObj.partyId;
            }
            this._primengProgressBarService.show();
            this.messagesApiFacadeService.feeheaderRegister(headerWageDto).subscribe((p) => {
                this._primengProgressBarService.hide();
                this.feeHeaderId = p.feeHeaderId;
                // if (this.wageObj.apiId == null) {
                this._primengProgressBarService.show();
                this.messagesApiFacadeService
                    .apifeeRegister2(this.partyId, this.feeHeaderId, this.clientId, this.apiId)
                    .subscribe((y) => {
                    this._primengProgressBarService.hide();
    ...
    deleted(wage) {
        this.functionalityWageType();
        const index = this.wageDetailList.findIndex((obj) => obj.row === wage.row);
        if (index > -1) {
    ...
    constAmontCheck(editFlag) {
        return this.wageDetailList.length == 1 && !editFlag;
    }
    validationRegister(editFlag) {
        if (!this.commonValidationsService.stringNotNullChecker(this.feeTitle)) {
            this.notifierService.showError({
                detail: 'لطفا عنوان را وارد کنید!',
                life: 3000,
        ...
        else if (this.calculationTypeId == 1 &&
            !this.commonValidationsService.numberNotNullChecker(this.calculationValue)) {
            this.notifierService.showError({
        ...
        else if ((this.feeTypeId == '3' || this.feeTypeId == '4') &&
            ...
            !this.commonValidationsService.numberNotNullWithoutZeroChecker(this.fromValue)) {
            this.notifierService.showError({
        ...
        else if (!this.commonValidationsService.numberNotNullWithoutZeroChecker(this.minValue)) {
            debugger;
            this.notifierService.showError({
        ...
        else if (!this.commonValidationsService.numberNotNullChecker(this.maxvalue)) {
            debugger;
            this.notifierService.showError({
    ...
    overlapChecked(newFromValue, newToValue, editFlag) {
        let counter = 0;
...
__decorate([
    Output()
], WageRegisterComponent.prototype, "close", void 0);
__decorate([
    Input()
], WageRegisterComponent.prototype, "wageObj", void 0);
WageRegisterComponent = __decorate([
    Component({
