...
let WageComponent = class WageComponent {
    router;
    route;
    transloco;
    ...
    constructor(router, route, transloco, messagesApiFacadeService, _primengProgressBarService, apiGatewayService, commonValidationsService, translateService, config, notifierService) {
        this.router = router;
        this.route = route;
    ...
    search() {
        if (this.partyId) {
            ...
            this.messagesApiFacadeService
                .apifeesearch(this.partyId, this.clientId, this.apiId, this.fromdate, this.todate)
                .subscribe((v) => {
                this._primengProgressBarService.hide();
                this.wageList = [];
                if (Array.isArray(v)) {
                ...
                for (let k = 0; k < this.wageList.length; k++) {
                    if ('row' in this.wageList) {
                        ...
                        this.wageList[k] = Object.assign(this.wageList[k], { row: k + 1 });
                    }
                }
            }, (error) => {
                this._primengProgressBarService.hide();
    ...
    OnchangeParty(event) {
        if (event.value != null) {
            ...
            this.search();
            this._primengProgressBarService.show();
            this.messagesApiFacadeService.apibypartyid(event.value).subscribe((l) => {
                this._primengProgressBarService.hide();
    ...
    validationAdd() {
        ...
        if (!this.commonValidationsService.stringNotNullChecker(this.clientId)) {
            this.notifierService.showError({
    ...
    showAdd() {
        if (this.validationAdd()) {
            ...
            if (this.apiId) {
                ...
                this._primengProgressBarService.show();
                this.messagesApiFacadeService
                    .getapicountwithoutfeebypartyandclientandapi(this.partyId, this.clientId, this.apiId)
                    .subscribe((t) => {
                    this._primengProgressBarService.hide();
                    if (t == 0) {
