...
let TestRunConditionComponent = class TestRunConditionComponent {
    router;
    route;
    ...
    partyBase;
    detailsBreadObject = [];
    constructor(router, route, messagesApiFacadeService, transloco, notifierService, validationsService, confirmationService, encod, apiGatewayService, cdr, _primengProgressBarService) {
        this.router = router;
        this.route = route;
    ...
    restoreCurlItem(key, type) {
        let backupList = [];
        if (type === 'header')
            backupList = this.curlBackup.headers;
        if (type === 'body')
            backupList = this.curlBackup.body;
        if (type === 'query')
            backupList = this.curlBackup.query;
  ...
  */
    extractBodyList(curl) {
        const result = [];
        // گرفتن body کامل
        const match = curl.match(/(?:--data-raw|--data|-d)\s+'([\s\S]*?)'/);
        if (!match) {
            return result;
        }
            ...
            next: (res) => {
                ...
                for (const condition of this.ruleConditionsList) {
                    ...
                    if (condition.conditionType === 1)
                        serviceBody.push(item);
                    if (condition.conditionType === 2)
                        serviceHeaders.push(item);
                    if (condition.conditionType === 3)
                        serviceQuery.push(item);
                }
    ...
    get bodyAsHtml() {
        const valid = this.curlBodyList.filter(b => b.key && b.value !== null && b.value !== undefined);
        if (!valid.length) {
    ...
    findInCurl(key, type) {
        let list = [];
        if (type === 'header')
            list = this.curlHeaderList;
        if (type === 'body')
            list = this.curlBodyList;
        if (type === 'query')
            list = this.curlQueryList;
    ...
    copyCurlToClipboard() {
        ...
        if (method !== 'GET') {
            const validBody = this.body.filter(b => b.key && b.value !== null && b.value !== undefined);
            if (validBody.length) {
 ...
 /!*        const missingBody = this.bodyList
    ...
    validateCurlBeforeSend() {
        ...
        const missingHeader = this.headerList
            .filter(h => h.enabled)
            .find(item => !this.curlHeaderList.some(c => c.key === item.key));
        if (missingHeader) {
            this.notifierService.showError(`فیلد هدر '${missingHeader.key}' در cURL یافت نشد`);
            return false;
        }
        ...
        const missingBody = this.bodyList
            ...
            if (!item.bodyNodePath ||
                item.bodyNodePath === '-' ||
                item.bodyNodePath.trim() === '') {
                return !Object.prototype.hasOwnProperty.call(bodyJson, item.key);
            }
            // اگر NodePath داشت
            const value = this.getValueByPath(bodyJson, item.bodyNodePath);
            return value === undefined;
        });
        if (missingBody) {
            this.notifierService.showError(`فیلد body '${missingBody.key}' در cURL یافت نشد`);
            return false;
        }
        // 5) اولین Query تیک‌خورده‌ای که در cURL وجود ندارد
        const missingQuery = this.queryList
            .filter(q => q.enabled)
            .find(item => !this.curlQueryList.some(c => c.key === item.key));
        if (missingQuery) {
            this.notifierService.showError(`فیلد query string '${missingQuery.key}' در cURL یافت نشد`);
            return false;
        }
        // اگر تا اینجا خطایی نبود
        return true;
...
__decorate([
    Output()
], TestRunConditionComponent.prototype, "close", void 0);
__decorate([
    Input()
], TestRunConditionComponent.prototype, "inputUpdate", void 0);
TestRunConditionComponent = __decorate([
    Component({
...
export { TestRunConditionComponent };
