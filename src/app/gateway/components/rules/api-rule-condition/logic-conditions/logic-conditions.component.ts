...
export class LogicConditionsComponent implements OnInit {
    ...
    @Input() inputLogic: any;
    allExpressionOptions: SelectOption[] = [];
    ...
    private normalizeOpCodeFromService(raw: number | string | null): any {
        ...
        switch (caseBase) {
            case 'rulesBase':
                return [
                    ...
                    {
                        index: 2,
                        label_index2: this.transloco.translate(
                            'breadcrumbs.rulesCondition',
                        ),
                        rout_index2: null,
                        isActive2: false,
                    ...
                    {
                        index: 3,
                        label_index3: this.transloco.translate(
                            'breadcrumbs.logicGates',
                        ),
            ...
            case 'ruleClientBase':
                return [
                    ...
                    },
                    {
                        ...
                        isActive2: false,
                        label_Detail_index2:
                            '(' + this.transloco.translate('menu.accessList') + ')',
                    ...
                    {
                        index: 4,
                        label_index4: this.transloco.translate(
                            'breadcrumbs.rulesCondition',
                        ),
                        rout_index4: null,
                        isActive4: true,
                    ...
                    {
                        index: 5,
                        label_index5: this.transloco.translate(
                            'breadcrumbs.logicGates',
                        ),
            ...
            case 'moduleBase':
                return [
                    {
                        index: 0,
                        label_index0: this.transloco.translate(
                            'breadcrumbs.serviceRecipients',
                        ),
            ...
            case 'partyBase':
                return [
                    ...
                    {
                        index: 6,
                        label_index6: this.transloco.translate(
                            'breadcrumbs.logicGates',
                        ),
    ...
    get filledBetweenCount(): number {
        return this.betweenValues.filter(v => !!v).length;
    }
    hasFreeExpression(): boolean {
        const usedValues = new Set<number | string>();
    ...
    private normalizeOp(raw: any): string | null {
        if (raw == null) return null;
        if (typeof raw === 'number') {
            if (raw === 1 || raw === 2 || raw === 3) return String(raw);
        }
        ...
        if (typeof raw === 'string') {
            ...
            if (lower === 'and') return '1';
            if (lower === 'or') return '2';
    ...
    private findOperatorCodeFromService(value: number | string | null): string {
        if (value == null) return null;
        const opt = this.OperatorOptions.find((o: any) => o.code == value);
    ...
    isCardComplete(cardIndex: number): boolean {
        ...
        if (cardIndex === 0) {
            ...
            if (hasFirst && !hasSecond && !hasThird) return true;
            if (hasFirst && hasSecond && hasThird) return true;
            return false;
        }
    }
    getAvailableExpressionOptions(cardIndex: number, selectIndex: number): SelectOption[] {
        debugger
        ...
        this.cards.forEach((card, i) => {
            card.selects.forEach((val, j) => {
                debugger
                if (val == null) return;
                if (i === cardIndex && j === selectIndex) return;
...
/*    removeCard(index: number): void {
 ...
 /*   private buildRuleConditions(ruleId: number): any[] {
    ...
    private buildRuleConditions(ruleId: number): LogicConditionDto[] {
        const result: LogicConditionDto[] = [];
        ...
        this.cards.forEach((card, index) => {
            ...
            if (left != null) {
                ...
                const leftDto: LogicConditionDto = {
                    ruleId,
            ...
            if (right != null) {
                ...
                const parentElementId =
                    left != null ? Number(left) : rightRuleConditionId;

                const rightDto: LogicConditionDto = {
                    ruleId,
    ...
    buildRuleConditionGroups(
        ruleId: number,
        elementIdMap: Map<number, number>
    ): RuleConditionGroupDto[] {
        const groups: RuleConditionGroupDto[] = [];
    ...
    mapFirstServiceToCards(rows: FirstServiceRow[]): SmartCard[] {
    ...
    mapSecondServiceToUI(rows: SecondServiceRow[]) {
        ...
        const cards: SmartCard[] = [];
        const betweenValues: (string | null)[] = [];
        const groupIds: (number | null)[] = [];  // NEW
    ...
    private mapOpNumberToCode(op: number | null): string | null {
        if (op === 1) return '1';
        if (op === 2) return '2';
        return null;
    }
    mapFirstServiceToOptions(rows: FirstServiceRow[]) {
        ...
        this.messagesApiFacadeService
            ...
            .subscribe(
                (res: LogicConditionDto[]) => {
                    ...
                    const groupsPayload = this.buildRuleConditionGroups(
                        this.ruleId,
                        elementIdMap,
                    );
                        ...
                        .subscribe(
                            () => {
                                this.notifierService.showSuccess(
                                    'شرایط و گروه‌ها با موفقیت ثبت شدند'
                                );
                                this.close.emit('closeAndCreate');
                            },
                            () => {
...
interface SmartCard {
    selects: [number | null, any, number | null];
    status: boolean;
    elementIdLeft?: number | null;
    elementIdRight?: number | null;
}
interface FirstServiceRow {
    id: number;
    ruleId: number;
    ruclecondicionId: number;
...
interface SecondServiceRow {
    ruleConditionIdOne: number | null;
    ruleConditionIdTwo: number | null;
    ruleConditionOneExpression: string;
    ruleConditionTwoExpression: string;
    ruleConditionOp: number | null;
    ruleConditionGroupOp: number | null;
    pairGroupId: number;
    groupId?: number | null;       // NEW
    elementIdOne?: number | null;  // NEW
    elementIdTwo?: number | null
}
interface RuleConditionGroupDto {
    id?: number;
    ruleconditionElementFirstId: number | null;
    ruleconditionElementSecondId: number | null;
    ruleconditionElementThirdId: number | null;
    ruleconditionElementFourthId: number | null;
    op: string | null;
    ruleId: number;
}
