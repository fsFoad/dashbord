...
export class AlertAccessLevelComponent implements OnInit {
    ...
    toggleSelection(item: any, type: 'source' | 'target') {
    ...
    isSelected(item: any, type: 'source' | 'target'): boolean {
        const list = type === 'source' ? this.selectedSourceItems : this.selectedTargetItems;
 ...
 /*   loadSectionReceiver() {
        ...
        this.messagesApiFacadeService.getSectionReceiver(0, 10000).subscribe({
            next: (res: any) => {
                ...
                const receiverMap = new Map<number, any>(
                    (this.availableReceivers ?? []).map((r: any) => [
                ...
                const sectionMap = new Map<number, any>(
                    (this.availableErrors ?? []).map((s: any) => [
                        Number(s.id),
                        s
                    ])
                );
                ...
                const grouped: Grouped = enriched.reduce<Grouped>((acc, curr: any) => {
    ...
    private findParentInTree(receiverId: any) {
        if (!receiverId) return null;
        return (this.treeData() ?? []).find((p: any) => p?.data?.receiverId === receiverId) ?? null;
    }
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                if (isParentNode) {
                    ...
                    (children.length ? children : sectionIds).forEach((child: any) => {
    ...
    deleteSectionReceiver(sectionId: string, receiverId: string) {
        return this.messagesApiFacadeService.sectionReceiverStatusEffect(0, sectionId, receiverId).pipe(
            tap(() => {
    ...
    addChildRowNumbers(children: TreeNode[], prefix: string) {
...
interface GroupBucket {
    ...
    sectionIdSet: Set<string | number>;
    children: TreeNode[];
    hasActiveChild: boolean;
}

type Grouped = Record<string, GroupBucket>;
