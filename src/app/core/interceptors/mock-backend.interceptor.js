...
import { ACTIVITY, COMMENTS, EVENTS, FILES, MEMBERS, NOTIFICATIONS, PROJECTS, TASKS, TEAM, USERS, nextEventId, nextFileId, toPublicUser, } from '../mock/mock-db';
...
export const mockBackendInterceptor = (req, next) => {
    ...
    if (!environment.useMock || !req.url.startsWith('/api/'))
        return next(req);
    const ok = (body, ms = rand(250, 650)) => of(new HttpResponse({ status: 200, body })).pipe(delay(ms));
    const fail = (status, ms = rand(150, 400)) => timer(ms).pipe(switchMap(() => throwError(() => new HttpErrorResponse({ status, statusText: 'Mock Error', url: req.url }))));
    ...
    // --- demo endpoints ---
    if (path === '/api/demo/network')
        return fail(0);
    const errMatch = path.match(/^\/api\/demo\/error\/(\d+)$/);
    if (errMatch)
        return fail(Number(errMatch[1]));
    ...
    if (path === '/api/auth/login' && req.method === 'POST') {
        ...
        if (!found)
            return fail(401, 600);
        const body = { token: `mock-${found.id}`, user: toPublicUser(found) };
        return ok(body, 600);
    }
    if (path === '/api/auth/verify-2fa' && req.method === 'POST') {
        ...
        if (!uid)
            return fail(401);
        if (code !== '111111')
            return fail(422); // wrong OTP (demo code: all ones)
        PENDING_2FA.delete(challengeId);
        const user = USERS.find((u) => u.id === uid);
        const body = { token: `mock-${user.id}`, user: toPublicUser(user) };
        return ok(body, 400);
    }
    ...
    if (path === '/api/auth/verify-otp' && req.method === 'POST') {
        const { challengeId, code } = (req.body ?? {});
        if (!challengeId || !PENDING_RESET.has(challengeId))
            return fail(401);
        if (code !== '111111')
            return fail(422); // wrong OTP (demo code: all ones)
    ...
    if (path === '/api/projects' && req.method === 'GET') {
        ...
        let filtered = PROJECTS.filter((p) => (!q || p.name.includes(q) || p.owner.includes(q)) &&
    ...
    if (updMatch && req.method === 'PUT') {
        ...
        const b = (req.body ?? {});
        if (b.name !== undefined)
            p.name = String(b.name).trim() || p.name;
        if (b.status !== undefined)
            p.status = b.status;
        if (b.progress !== undefined)
            p.progress = Math.min(100, Math.max(0, Number(b.progress)));
    ...
    if (path === '/api/team' && req.method === 'POST') {
        ...
        if (!b.name?.trim() || !b.email?.trim())
            return fail(400);
        if (TEAM.some((m) => m.email === b.email))
            return fail(409);
        const member = {
    ...
    if (eventMatch && req.method === 'PUT') {
        ...
        const b = (req.body ?? {});
        if (b.title !== undefined)
            e.title = String(b.title).trim() || e.title;
        if (b.date !== undefined)
            e.date = b.date;
    ...
    if (path === '/api/ai/summary' && req.method === 'POST') {
        ...
        const overdue = PROJECTS.filter((p) => p.status !== 'done' && new Date(p.dueDate).getTime() < Date.now()).length;
        const avgProgress = Math.round(PROJECTS.reduce((s, p) => s + p.progress, 0) / Math.max(1, PROJECTS.length));
    ...
    if (path === '/api/stats' && req.method === 'GET') {
        ...
        const series = (n, min, max) => Array.from({ length: n }, () => Math.round(min + next() * (max - min)));
        const byStatus = {
