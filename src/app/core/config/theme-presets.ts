/** A 50–950 color ramp. Values may be hex or PrimeNG token refs like '{indigo.500}'. */
 ...
 */
function ramp(name: string): Palette {
  return {
    50: `{${name}.50}`,
    100: `{${name}.100}`,
    200: `{${name}.200}`,
...
export interface ThemePreset {
  key: string;
  /** Swatch color (hex) shown in the settings panel. */
  swatch: string;
  /** Primary palette as token references. */
  palette: Palette;
}
...
/** Neutral "surface" palettes — control the grays of the whole UI. */
...
export function findSurface(key: string): SurfacePalette {
