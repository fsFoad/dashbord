...
export const PROJECTS = NAMES.map((name, i) => {
...
export const MEMBERS = OWNERS.map((name, i) => ({ id: i + 1, name }));
...
for (let p = 1; p <= 8; p++) {
    ...
    for (let t = 0; t < count; t++) {
        ...
        TASKS.push({
            ...
            status: ['todo', 'doing', 'done'][(p + t) % 3],
            priority: ['low', 'medium', 'high'][(p * 2 + t) % 3],
...
for (let p = 1; p <= 8; p++) {
    COMMENTS.set(p, COMMENT_TEXTS.slice(0, 2 + (p % 3)).map((text, k) => ({
        id: commentSeq++,
...
// ---------------- Team ----------------
export const TEAM = OWNERS.map((name, i) => ({
    id: i + 1,
    name,
    email: ['sara', 'ali', 'maryam', 'hossein', 'negar'][i] + '@demo.com',
    role: ['admin', 'manager', 'member', 'member', 'member'][i],
    active: i !== 3,
...
export const LIVE_NOTIF_TEMPLATES = [
...
// ---------------- File manager ----------------
let fileSeq = 1;
const F = (parentId, name, type, size = null) => ({
...
FILES.push(fDocs, fImgs, fContracts, F(null, 'برنامه-فازبندی.xlsx', 'file', 48_500), F(fDocs.id, 'پروپوزال-نهایی.pdf', 'file', 1_240_000), F(fDocs.id, 'صورتجلسه-خرداد.docx', 'file', 86_000), F(fDocs.id, 'گزارش‌ها', 'folder'), F(fImgs.id, 'لوگو.png', 'file', 210_000), F(fImgs.id, 'بنر-کمپین.jpg', 'file', 2_400_000), F(fContracts.id, 'قرارداد-اصلی.pdf', 'file', 540_000));
