...
const keyFor = (uid: number) => `app.tour.${uid}`;
...
export class TourService {
  ...
  maybeAutoStart(): void {
    ...
    if (!u || this.storage.read(keyFor(u.id), false)) return;
  ...
  next(): void {
    const i = this.index();
    if (i === null) return;
    if (i + 1 >= STEPS.length) this.finish();
    else this.goTo(i + 1);
  }

  prev(): void {
    const i = this.index();
    if (i !== null && i > 0) this.goTo(i - 1);
  }
  ...
  finish(): void {
    ...
    this.rect.set(null);
    const u = this.session.user();
    if (u) this.storage.write(keyFor(u.id), true);
  }

  remeasure(): void {
    const s = this.step();
    if (!s) return;
    const el = this.doc.querySelector<HTMLElement>(`[data-tour="${s.target}"]`);
    this.rect.set(el ? el.getBoundingClientRect() : null);
  }

  private goTo(i: number, hops = 0): void {
    if (hops >= STEPS.length) {
      this.finish();
      return;
    }
    const el = this.doc.querySelector<HTMLElement>(`[data-tour="${STEPS[i].target}"]`);
    if (!el) {
      // target not on screen (e.g. mobile) → skip forward
      const ni = i + 1;
      if (ni >= STEPS.length) this.finish();
      else this.goTo(ni, hops + 1);
      return;
    }
    this.index.set(i);
